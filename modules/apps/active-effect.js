...
export default class WFRPActiveEffectConfig extends ActiveEffectConfig {
    ...
    getData() {
        ...
        data.effectTriggers = game.wfrp2e.config.effectTriggers;
        let type = getProperty(data, "effect.flags.wfrp2e.effectTrigger")
        if (type && type != "dialogChoice")
        {
            data.showEditor = true;
            data.placeholder = game.wfrp2e.config.effectPlaceholder[type] + game.wfrp2e.config.effectPlaceholder.this
        }

        data.effectApplication = duplicate(game.wfrp2e.config.effectApplication)
        ...
        {
            data.effect.flags.wfrp2e.effectTrigger = "applyDamage"
    ...
    get template() {
        return "systems/wfrp2e/templates/apps/active-effect-config.html"
    }
