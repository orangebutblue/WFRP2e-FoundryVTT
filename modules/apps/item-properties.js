export default class ItemProperties extends FormApplication {
    static get defaultOptions() {
        ...
        options.template = "systems/wfrp2e/templates/apps/item-properties.html";
        options.height = "auto";
        options.width = 400;
    ...
    constructor(...args) {
        ...
        if (this.object.type == "weapon" || this.object.type == "ammunition") {
            this.qualities = foundry.utils.deepClone(game.wfrp2e.config.weaponQualities)
            this.flaws = foundry.utils.deepClone(game.wfrp2e.config.weaponFlaws)
        }
        else if (this.object.type == "armour") {
            this.qualities = foundry.utils.deepClone(game.wfrp2e.config.armorQualities)
            this.flaws = foundry.utils.deepClone(game.wfrp2e.config.armorFlaws)
        }
        ...
        mergeObject(this.qualities, game.wfrp2e.config.itemQualities)
        mergeObject(this.flaws, game.wfrp2e.config.itemFlaws)
    }
    ...
    getData() {
        ...
        data.qualities = Object.keys(this.qualities).map(i => {
            return {
                ...
                hasValue: game.wfrp2e.config.propertyHasValue[i],
                key: i,
