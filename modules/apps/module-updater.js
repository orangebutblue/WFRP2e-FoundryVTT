...
export default class ModuleUpdater extends Dialog {
    ...
    {
        let html = await renderTemplate("systems/wfrp2e/templates/apps/module-updater.html", module)
        ...
        for (let document of documents)
        {
            ...
            if (game[document.collectionName].has(document.id))
            {
                ...
                if (!settings.excludeNameChange || (settings.excludeNameChange && document.name == existingDoc.name))
                {
                    ...
                    game.wfrp2e.utility.log(`Updated Document ${document.name}`)
                    this.count.updated++;
                }
            }
            else
            {
                ...
                toCreate.push(newDoc)
                game.wfrp2e.utility.log(`Imported Document ${document.name}`)
                this.count.created++;
            }
        }
