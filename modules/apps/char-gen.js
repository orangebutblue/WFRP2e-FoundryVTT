 ...
 */
export default class GeneratorWfrp2e {
  ...
  static start()
  {
    game.wfrp2e.generator = new this()
  }

  speciesStage() {
    if (! game.wfrp2e.config.species)
      return ui.notifications.error("No content found")

    renderTemplate("systems/wfrp2e/templates/chat/chargen/species-select.html", { species:  game.wfrp2e.config.species }).then(html => {
  ...
  async rollSpecies(messageId, chosenSpecies) {
    ...
    if (chosenSpecies) {
      this.speciesExp = 0;
      roll = { roll: game.i18n.localize("Choose"), species: chosenSpecies, name:  game.wfrp2e.config.species[chosenSpecies]}
    }
    else {
      this.speciesExp = 20;
      roll = await game.wfrp2e.tables.rollTable("species");
    }

    this.species = roll.species

    let speciesMessage = game.messages.get(messageId)
    let updateCardData = { roll: roll, species:  game.wfrp2e.config.species }
    ...
    if (game.wfrp2e.config.subspecies[roll.species])
    {
      return renderTemplate("systems/wfrp2e/templates/chat/chargen/subspecies-select.html", { species: roll.species, speciesDisplay : game.wfrp2e.config.species[roll.species], subspecies:  game.wfrp2e.config.subspecies[roll.species]}).then(html => {
        let chatData = WFRP_Utility.chatDataSetup(html)
        ChatMessage.create(chatData);
      })
    }
  ...
  async rollAttributes(reroll = false) {
    ...
    let dataTransfer = {
      ...
      payload : {
        ...
        movement:  game.wfrp2e.config.speciesMovement[species],
        fate:  game.wfrp2e.config.speciesFate[species],
        resilience:  game.wfrp2e.config.speciesRes[species],
    ...
    for (let abrev in  game.wfrp2e.config.characteristicsAbbrev) {
      cardData.characteristics[ game.wfrp2e.config.characteristicsAbbrev[abrev]] = dataTransfer.payload.characteristics[abrev]
    }
    cardData.speciesKey = species;
    cardData.species = game.wfrp2e.config.species[species]
    if (this.subspecies)
      cardData.species += ` (${game.wfrp2e.config.subspecies[species][this.subspecies].name})`
    cardData.extra =  game.wfrp2e.config.speciesExtra[species]
    cardData.move =  game.wfrp2e.config.speciesMovement[species]
  ...
  async speciesSkillsTalents() {
    ...
    let cardData = {
      speciesKey: species,
      species:  game.wfrp2e.config.species[species],
    ...
    {
      let talent = await game.wfrp2e.tables.rollTable("talents")
  ...
  async rollCareer(isReroll=false) {
    this.careerExp = 0
    if (isReroll)
      this.careerExp = game.wfrp2e.config.randomExp.careerReroll
    else
      this.careerExp = game.wfrp2e.config.randomExp.careerRand

    let rollSpecies = this.species
    if (this.species == "human" && !this.subspecies)
      this.subspecies = "reiklander"
    if (this.subspecies && game.wfrp2e.tables.findTable("career", rollSpecies + "-" + this.subspecies))
      rollSpecies += "-" + this.subspecies
    let roll = await game.wfrp2e.tables.rollTable("career", {}, rollSpecies)
  ...
  async chooseCareer() {
    ...
    let rollSpecies = this.species;
    let table = game.wfrp2e.tables.findTable("career", rollSpecies)
    if (this.subspecies && game.wfrp2e.tables.findTable("career", rollSpecies + "-" + this.subspecies))
    {
      rollSpecies += "-" + this.subspecies
      table = game.wfrp2e.tables.findTable("career", rollSpecies)
    }
  ...
  async displayCareer(careerName, isReroll, isChosen) {
    let packs = game.wfrp2e.tags.getPacksWithTag("career")
    ...
    let cardData = {
      ...
      trappings:  game.wfrp2e.config.classTrappings[WFRP_Utility.matchClosest( game.wfrp2e.config.classTrappings, careerFound.data.data.class.value, {matchKeys: true})] // Match closest is needed here (Academics/Academic)
    }
  ...
  async rollDetails(species) {
    ...
    eyes = (await game.wfrp2e.tables.rollTable("eyes", {}, species)).result
    hair = (await game.wfrp2e.tables.rollTable("hair", {}, species)).result

    age = (await new Roll( game.wfrp2e.config.speciesAge[species]).roll()).total;
    heightRoll = (await new Roll( game.wfrp2e.config.speciesHeight[species].die).roll()).total;
    hFeet =  game.wfrp2e.config.speciesHeight[species].feet;
    hInches =  game.wfrp2e.config.speciesHeight[species].inches + heightRoll;
