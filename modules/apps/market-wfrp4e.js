 ...
 */
export default class MarketWfrp2e {
    /**
    ...
    static async testForAvailability({ settlement, rarity, modifier }) {
        //This method read the table  game.wfrp2e.config.availabilityTable defined in the config file

        //First we get the different settlements size
        let validSettlements = Object.getOwnPropertyNames( game.wfrp2e.config.availabilityTable);
        ...
        if (settlement && validSettlementsLocalized.hasOwnProperty(settlement)) {
            let validRarity = Object.getOwnPropertyNames( game.wfrp2e.config.availabilityTable[validSettlementsLocalized[settlement]]);
        ...
        //Everything is ok, lets roll for availability
            ...
            let availabilityLookup =  game.wfrp2e.config.availabilityTable[validSettlementsLocalized[settlement]][validRarityLocalized[rarity]];
    ...
    static generateSettlementChoice(rarity) {
        let cardData = { rarity:  game.wfrp2e.config.availability[rarity] };
        renderTemplate("systems/wfrp2e/templates/chat/market/market-settlement.html", cardData).then(html => {
     ...
     * @param {Array} moneyItemInventory
     * @param transactionType  game.wfrp2e.config.transactionType, is it a payment or an income
     */
    ...
    static throwMoney(moneyValues)
    {
        ...
        if (game.dice3d && game.settings.get("wfrp2e", "throwMoney")){
    ...
    static generatePayCard(payRequest, player) {
        ...
        } else //generate a card with a summary and a pay button
        {
            ...
            renderTemplate("systems/wfrp2e/templates/chat/market/market-pay.html", cardData).then(html => {
     ...
     * @param { game.wfrp2e.config.creditOptions} option
     */
    static generateCreditCard(creditRequest, option = "EACH") {
        ...
        } else //generate a card with a summary and a receive button
        {
            ...
            else if (option.toLowerCase() ===  game.wfrp2e.config.creditOptions.SPLIT.toLowerCase()) {
            ...
            else if (option.toLowerCase() ===  game.wfrp2e.config.creditOptions.EACH.toLowerCase()) {
            ...
            renderTemplate("systems/wfrp2e/templates/chat/market/market-credit.html", cardData).then(html => {
