export default class WFRP2eTableSheet extends Application {
  constructor(table, app) {
    super(app)

    if (typeof table == "string")
    {
      this.table = duplicate(game.wfrp2e.tables[game.wfrp2e.tables.generalizeTable(table)])
  ...
  static get defaultOptions() {
    ...
    options.id = "wfrp2e-table";
    options.template = "systems/wfrp2e/templates/apps/table-sheet.html"
    options.classes.push("wfrp2e", "table-sheet");
    options.resizable = true;
    options.height = 900;
    options.width = 600;
    options.minimizable = true;
    options.title = "WFRP2e Table";
  ...
  getData() {
    ...
    data.tableVisibility = game.settings.get("wfrp2e", "tableVisibility")[this.table.key]
    ...
    data.rollMode = game.settings.get("wfrp2e", "tableRollMode")[this.table.key] || game.settings.get("core", "rollMode")
    return data;
  }
  ...
  activateListeners(html) {
    ...
    html.find(".roll-table-button").click(ev => {
      ...
      game.wfrp2e.tables.rollToChat(this.table.key, options, null, options.rollMode)
    })
    ...
    html.find(".row-label").click(ev => {
      ...
      game.wfrp2e.tables.rollToChat(this.table.key, options, column, options.rollMode)
    })

    html.find(".table-visibility").change(ev => {
      let tableVisibility = duplicate(game.settings.get("wfrp2e", "tableVisibility"))
      ...
      game.settings.set("wfrp2e", "tableVisibility", tableVisibility);
    })

    html.find(".table-roll-mode").change(ev => {
      let tableRollMode = duplicate(game.settings.get("wfrp2e", "tableRollMode"))

      tableRollMode[this.table.key] = ev.target.value

      game.settings.set("wfrp2e", "tableRollMode", tableRollMode);
    })
  }
  ...
  {
    let sheet = $(ev.currentTarget).parents(".wfrp2e-table")
    let options = {}
