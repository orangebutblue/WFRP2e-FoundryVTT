...
*/
export default class BrowserWfrp2e extends Application {
  constructor(app) {
    super(app)
  ...
  getData() {
    ...
    data.relations = ["<", "<=", "==", ">=", ">"]
    data.availability =  game.wfrp2e.config.availability;
    data.ammunitionGroups =  game.wfrp2e.config.ammunitionGroups;
    data.locations = ["Head", "Body", "Arm", "Leg"];
    data.mutationTypes =  game.wfrp2e.config.mutationTypes;
    data.armorTypes =  game.wfrp2e.config.armorTypes;
    data.gods = this.gods;
    data.weaponGroups =  game.wfrp2e.config.weaponGroups
    data.weaponReaches =  game.wfrp2e.config.weaponReaches;
    data.talentMax =  game.wfrp2e.config.talentMax;
    data.trappingTypes =  game.wfrp2e.config.trappingTypes;
    data.lores = this.lores;
    data.characteristics =  game.wfrp2e.config.characteristicsAbbrev;
    data.skillTypes =  game.wfrp2e.config.skillTypes
    data.skillGroup =  game.wfrp2e.config.skillGroup
    data.prayerTypes =  game.wfrp2e.config.prayerTypes;
  ...
  addItems(itemList) {
    ...
    this.lores = this.lores.map(p => {
      if ( game.wfrp2e.config.magicLores[p])
        return  game.wfrp2e.config.magicLores[p];
      else
        return p;
    })
    ...
    for (let filter in this.filters.dynamic) {
      if (this.filters.dynamic[filter].show && this.filters.dynamic[filter].value) {
        switch (filter) {
          ...
          case "melee":
          case "ranged":
            filteredItems = filteredItems.filter(i => i.type != "weapon" || filter ==  game.wfrp2e.config.groupToType[i.data.data.weaponGroup.value])
            break;
          case "weaponRange":
