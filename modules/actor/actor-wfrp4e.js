...
import WFRP_Audio from "../system/audio-wfrp2e.js";
import RollDialog from "../apps/roll-dialog.js";
import EffectWfrp2e from "../system/effect-wfrp2e.js"
 ...
 * @see   ChatWFRP2e - Sends test data to roll tests.
 */
export default class ActorWfrp2e extends Actor {
  ...
  async _preCreate(data, options, user) {
    ...
    if (!data.img) {
      ...
      if (data.type == "vehicle")
        createData.img = "systems/wfrp2e/tokens/vehicle.png"
    }
  ...
  async _preUpdate(updateData, options, user) {
    ...
    if (this.data.token.img == "systems/wfrp2e/tokens/unknown.png" && updateData.img) {
  ...
  get effects() {
    ...
    effects.forEach(e => {
      ...
      try {
        ...
        game.wfrp2e.utility.log(`The effect ${e.label} threw an error when being prepared. ${error}`, e)
      }
    })
    return actorEffects;
  ...
  prepareNonVehicle() {
    ...
    if (trait)
      size = WFRP_Utility.findKey(trait.specification.value, game.wfrp2e.config.actorSizes);
    ...
    if (this.isMounted && !game.actors) {
      game.wfrp2e.postReadyPrepare.push(this);
    }
 ...
 */
  prepareCharacter() {
    ...
    if (currentCareer) {
      ...
      this.details.status.value = game.wfrp2e.config.statusTiers[this.details.status.tier] + " " + this.details.status.standing
    }
    else
  ...
  async setupDialog({ dialogOptions, testData, cardOptions }) {
    ...
    dialogOptions.data.difficultyLabels = game.wfrp2e.config.difficultyLabels;
  ...
  setupCharacteristic(characteristicId, options = {}) {
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.CharacteristicTest,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/characteristic-dialog.html",
      ...
      callback: (html) => {
        ...
        testData.testDifficulty = game.wfrp2e.config.difficultyModifiers[html.find('[name="testDifficulty"]').val()];
    ...
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/characteristic-card.html", title)
  ...
  setupSkill(skill, options = {}) {
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.SkillTest,
      hitLocation: false,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/skill-dialog.html",
      // Prefilled dialog data
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/skill-card.html", title)
  ...
  setupWeapon(weapon, options = {}) {
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.WeaponTest,
      hitLocation: true,
    ...
    if (weapon.attackType == "melee")
      ...
      if (weapon.consumesAmmo.value && weapon.ammunitionGroup.value != "none" && weapon.ammunitionGroup.value) {
        ...
        if (!testData.ammo || !weapon.currentAmmo.value || testData.ammo.quantity.value == 0) {
          AudioHelper.play({ src: `${game.settings.get("wfrp2e", "soundPath")}no.wav` }, false)
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/weapon-dialog.html",
      // Prefilled dialog data
      data: {
        hitLocation: testData.hitLocation,
        talents: this.getTalentTests(),
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/weapon-card.html", title)
  ...
  setupCast(spell, options = {}) {
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.CastTest,
      item: spell.id,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/spell-dialog.html",
      // Prefilled dialog data
      data: {
        hitLocation: testData.hitLocation,
    ...
    //@HOUSE
    if (game.settings.get("wfrp2e", "mooMagicAdvantage")) {
      game.wfrp2e.utility.logHomebrew("mooMagicAdvantage")
      dialogOptions.data.advantage = "N/A"
    }
    //@/HOUSE

    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/spell-card.html", title)
  ...
  setupChannell(spell, options = {}) {
    ...
    if (spell.wind && spell.wind.value) {
      ...
      defaultSelection = channellSkills.indexOf(channellSkills.find(x => x.name.includes(game.wfrp2e.config.magicWind[spellLore])));
    }
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.ChannelTest,
      item: spell.id,
      malignantInfluence: false,
      options: options,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/channel-dialog.html",
      // Prefilled dialog data
      data: {
        malignantInfluence: testData.malignantInfluence,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/channel-card.html", title)
  ...
  setupPrayer(prayer, options = {}) {
    ...
    let testData = { // Store this data to be used in the test logic
      title,
      rollClass: game.wfrp2e.rolls.PrayerTest,
      item: prayer.id,
      hitLocation: false,
      options: options,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/prayer-dialog.html",
      // Prefilled dialog data
      data: {
        hitLocation: testData.hitLocation,
        talents: this.getTalentTests(),
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/roll/prayer-card.html", title)
  ...
  setupTrait(trait, options = {}) {
    ...
    let title = options.title || game.wfrp2e.config.characteristics[trait.rollable.rollCharacteristic] + ` ${game.i18n.localize("Test")} - ` + trait.name;
    title += options.appendTitle || "";
    ...
    let testData = {
      title,
      rollClass: game.wfrp2e.rolls.TraitTest,
    ...
    let dialogOptions = {
      title: title,
      template: "/systems/wfrp2e/templates/dialog/skill-dialog.html", // Reuse skill dialog
      // Prefilled dialog data
      data: {
        hitLocation: testData.hitLocation,
        talents: this.getTalentTests(),
  ...
  setupExtendedTest(item, options = {}) {
    ...
    let characteristic = WFRP_Utility.findKey(item.test.value, game.wfrp2e.config.characteristics)
    if (characteristic) {
  ...
  _setupCardOptions(template, title) {
    ...
    if (this.token) {
      ...
      if (this.token.getFlag("wfrp2e", "mask")) {
        cardOptions.speaker.alias = "???"
        cardOptions.flags.img = "systems/wfrp2e/tokens/unknown.png"
      }
    }
    else // If a linked actor - use the currently selected token's data if the actor id matches
    {
      ...
      if (getProperty(this.data.token, "flags.wfrp2e.mask")) {
        cardOptions.speaker.alias = "???"
        cardOptions.flags.img = "systems/wfrp2e/tokens/unknown.png"
      }
    }
  ...
  rollReloadTest(weapon) {
    let testId = weapon.getFlag("wfrp2e", "reloading")
  ...
  _getTokenSize() {
    let tokenData = {}
    let tokenSize = game.wfrp2e.config.tokenSizes[this.details.size.value];
    if (tokenSize < 1)
 ...
 */
   ...
   */
  applyDamage(opposedTest, damageType = game.wfrp2e.config.DAMAGE_TYPE.NORMAL) {
    ...
    let applyAP = (damageType == game.wfrp2e.config.DAMAGE_TYPE.IGNORE_TB || damageType == game.wfrp2e.config.DAMAGE_TYPE.NORMAL)
    let applyTB = (damageType == game.wfrp2e.config.DAMAGE_TYPE.IGNORE_AP || damageType == game.wfrp2e.config.DAMAGE_TYPE.NORMAL)
    ...
    let messageElements = []
    // if (damageType !=  game.wfrp2e.config.DAMAGE_TYPE.IGNORE_ALL)
    //   updateMsg += " ("
    ...
    if (applyAP) {
      ...
      //@HOUSE
      if (penetrating && game.settings.get("wfrp2e", "mooPenetrating")) {
        game.wfrp2e.utility.logHomebrew("mooPenetrating")
      ...
      //@HOUSE
      if (game.settings.get("wfrp2e", "mooShieldAP") && opposedTest.defenderTest.result.outcome == "failure") {
        game.wfrp2e.utility.logHomebrew("mooShieldAP")
        shieldAP = 0;
      }
      //@/HOUSE
    ...
    Hooks.call("wfrp2e:applyDamage", scriptArgs)
    ...
    if (newWounds <= 0) {
      ...
      if (game.settings.get("wfrp2e", "dangerousCrits") && critAmnt && (Math.abs(newWounds) - actor.characteristics.t.bonus) > 0) {
      ...
      //@HOUSE
      else if (game.settings.get("wfrp2e", "mooCritModifiers")) {
        game.wfrp2e.utility.logHomebrew("mooCritModifiers")
        let critModifier = (Math.abs(newWounds) - actor.characteristics.t.bonus) * critAmnt;
        if (critModifier)
    ...
    if (hack)
      updateMsg += `<br>${game.i18n.localize("CHAT.DamageAP")} ${game.wfrp2e.config.locations[opposedTest.result.hitloc.value]}`
   ...
   */
  async applyBasicDamage(damage, { damageType = game.wfrp2e.config.DAMAGE_TYPE.NORMAL, minimumOne = true, loc = "body", suppressMsg = false } = {}) {
 ...
 */
  ...
  async _advanceSpeciesSkills() {
    ...
    try {
      let { skills } = game.wfrp2e.utility.speciesSkillsTalents(this.details.species.value, this.details.species.subspecies)
      skillList = skills
      if (!skillList) {
      ...
      console.log("wfrp2e | Could not find species " + this.details.species.value + ": " + error);
      throw error
    }
  ...
  async _advanceSpeciesTalents() {
    ...
    let talentList
    try {
      let { talents } = game.wfrp2e.utility.speciesSkillsTalents(this.details.species.value, this.details.species.subspecies)
      talentList = talents
      if (!talentList) {
      }
    }
    ...
    for (let talent of talentList) {
      if (!isNaN(talent)) // If is a number, roll on random talents
      {
        for (let i = 0; i < talent; i++) {
          let result = await game.wfrp2e.tables.rollTable("talents")
        ...
        endurance: {
          ...
          callback: () => {
            ...
            if (skill) {
              ...
              this.setupCharacteristic("t", { title: game.i18n.format("DIALOG.CorruptionTestTitle", { test: game.wfrp2e.config.characteristics["t"] }), corruption: strength }).then(setupData => this.basicTest(setupData))
            }
          }
        },
        cool: {
          ...
          callback: () => {
            ...
            if (skill) {
              ...
              this.setupCharacteristic("wp", { title: game.i18n.format("DIALOG.CorruptionTestTitle", { test: game.wfrp2e.config.characteristics["wp"] }), corruption: strength }).then(setupData => this.basicTest(setupData))
            }
          }
        }
  ...
  getPrefillData(type, item, options = {}) {
    ...
    try {
      ...
      if (this.type != "vehicle") {
        ...
        // @HOUSE
        if (type == "channelling" && game.settings.get("wfrp2e", "mooMagicAdvantage"))
          addAdvantage = true;

        if (type == 'cast' && game.settings.get("wfrp2e", "mooMagicAdvantage"))
          addAdvantage = false
        // @/HOUSE
        ...
        if (addAdvantage) {
          ...
          if (!game.settings.get("wfrp2e", "mooAdvantage")) {
            modifier += game.settings.get("wfrp2e", "autoFillAdvantage") ? (this.status.advantage.value * game.settings.get("wfrp2e", "advantageBonus") || 0) : 0
            if (parseInt(this.status.advantage.value) && game.settings.get("wfrp2e", "autoFillAdvantage"))
              tooltip.push(game.i18n.localize("Advantage"))
          }
          // @HOUSE - +1 Success Bonus SL per advantage
          else if (game.settings.get("wfrp2e", "mooAdvantage")) {
            successBonus += game.settings.get("wfrp2e", "autoFillAdvantage") ? (this.status.advantage.value * 1 || 0) : 0
            if (parseInt(this.status.advantage.value) && game.settings.get("wfrp2e", "autoFillAdvantage"))
              tooltip.push(game.i18n.localize("Advantage"))
          }
          // @/HOUSE
      ...
      if (attacker && attacker.test.weapon && attacker.test.weapon.properties.flaws.slow) {
        if (!game.settings.get("wfrp2e", "mooQualities") || ((type == "skill" && item.name == game.i18n.localize("NAME.Dodge")) || (type == "characteristic" && options.dodge))) {
          slBonus += 1
      ...
      if (options.modify) {
        ...
        if (options.modify.difficulty)
          difficulty = game.wfrp2e.utility.alterDifficulty(difficulty, options.modify.difficulty)
  ...
  rangePrefillModifiers(weapon, options, tooltip = []) {
    ...
    if (!game.settings.get("wfrp2e", "rangeAutoCalculation") || !token || !game.user.targets.size == 1 || !weapon.range?.bands)
      return 0
  ...
  sizePrefillModifiers(item, type, options, tooltip) {
    ...
    try {
      ...
      if (attacker) {
        //Size Differences
        let sizeDiff = game.wfrp2e.config.actorSizeNums[attacker.test.size] - this.sizeNum
        ...
        if (sizeDiff < 0 && (item.attackType == "melee" || target.sizeNum <= 3)) {
          ...
          if (target.sizeNum > 3 || target.sizeNum < 3)
            tooltip.push(game.i18n.format('CHAT.TestModifiers.ShootingSizeModifier', { size: game.wfrp2e.config.actorSizes[target.details.size.value] }))
        }
      }
  ...
  runEffects(trigger, args, options = {}) {
    ...
    if (trigger == "targetPrefillDialog" && game.user.targets.size) {
      ...
      let secondaryEffects = game.user.targets.values().next().value.actor.effects.filter(e => getProperty(e.data, "flags.wfrp2e.secondaryEffect.effectTrigger") == "targetPrefillDialog" && !e.isDisabled) // A kludge that supports 2 effects. Specifically used by conditions
      effects = effects.concat(secondaryEffects.map(e => {
        let newEffect = e.toObject()
        newEffect.flags.wfrp2e.effectTrigger = newEffect.flags.wfrp2e.secondaryEffect.effectTrigger;
        newEffect.flags.wfrp2e.script = newEffect.flags.wfrp2e.secondaryEffect.script;
        return new EffectWfrp2e(newEffect, { parent: e.parent })
      }))
    }
  ...
  async decrementInjury(injury) {
    ...
    if (injury.data.duration.value == 0) {
      let chatData = game.wfrp2e.utility.chatDataSetup(game.i18n.format("CHAT.InjuryFinish", { injury: injury.name }), "gmroll")
  ...
  async decrementDisease(disease) {
    ...
    if (!d.data.duration.active) {
      if (Number.isNumeric(d.data.incubation.value)) {
        ...
        let chatData = game.wfrp2e.utility.chatDataSetup(`Attempted to decrement ${d.name} incubation but value is non-numeric`, "gmroll", false)
        chatData.speaker = { alias: this.name }
        ChatMessage.create(chatData)
      }
    }
      ...
      if (Number.isNumeric(d.data.duration.value)) {
        ...
        let chatData = game.wfrp2e.utility.chatDataSetup(`Attempted to decrement ${d.name} duration but value is non-numeric`, "gmroll", false)
        chatData.speaker = { alias: this.name }
        ChatMessage.create(chatData)
      }
    }
  ...
  async activateDisease(disease) {
    ...
    let chatData = game.wfrp2e.utility.chatDataSetup(msg, "gmroll", false)
    chatData.speaker = { alias: this.name }
    ChatMessage.create(chatData)
  }
        ...
        this.setupSkill("Endurance", { difficulty }).then(setupData => this.basicTest(setupData).then(async test => {
          if (test.result.outcome == "failure") {
            ...
            if (negSL <= 1) {
              ...
              fromUuid("Compendium.wfrp2e-core.diseases.kKccDTGzWzSXCBOb").then(disease => {
                this.createEmbeddedDocuments("Item", [disease.toObject()])
              })
            }
            else if (negSL >= 6) {
              msg += game.i18n.localize("CHAT.LingeringRot")
              fromUuid("Compendium.wfrp2e-core.diseases.M8XyRs9DN12XsFTQ").then(disease => {
                this.createEmbeddedDocuments("Item", [disease.toObject()])
              })
            }
          }
  ...
  async handleIncomeTest(roll) {
    ...
    let dieAmount = game.wfrp2e.config.earningValues[tier] // b, s, or g maps to 2d10, 1d10, or 1 respectively (takes the first letter)
  ...
  async checkCorruption() {
    ...
    if (this.status.corruption.value > this.status.corruption.max) {
      ...
      if (skill) {
        ...
        this.setupCharacteristic("t", { title: game.i18n.format("DIALOG.MutateTitle", { test: game.wfrp2e.config.characteristics["t"] }), mutate: true }).then(setupData => {
          this.basicTest(setupData)
        });
      }
    }
  ...
  async handleMutationResult(test) {
    ...
    if (failed) {
      let wpb = this.characteristics.wp.bonus;
      let tableText = game.i18n.localize("CHAT.MutateTable") + "<br>" + game.wfrp2e.config.corruptionTables.map(t => `@Table[${t}]<br>`).join("")
  ...
  async handleExtendedTest(test) {
    ...
    if (game.settings.get("wfrp2e", "extendedTests") && test.result.SL == 0)
    ...
    if (item.data.SL.current >= item.data.SL.target) {
      ...
      if (getProperty(item, "flags.wfrp2e.reloading")) {
        let actor
        if (getProperty(item, "flags.wfrp2e.vehicle"))
          actor = WFRP_Utility.getSpeaker(getProperty(item, "flags.wfrp2e.vehicle"))

        actor = actor ? actor : this
        let weapon = actor.items.get(getProperty(item, "flags.wfrp2e.reloading"))
        weapon.update({ "flags.wfrp2e.-=reloading": null, "data.loaded.amt": weapon.loaded.max, "data.loaded.value": true })
      }
  ...
  checkReloadExtendedTest(weapon) {
    ...
    let reloadingTest = this.items.get(weapon.getFlag("wfrp2e", "reloading"))

    if (weapon.loaded.amt > 0) {
      if (reloadingTest) {
        reloadingTest.delete()
        weapon.update({ "flags.wfrp2e.-=reloading": null })
        return ui.notifications.notify(game.i18n.localize("ITEM.ReloadFinish"))
      }
    }
    else {
      let reloadExtendedTest = duplicate(game.wfrp2e.config.systemItems.reload);
      ...
      reloadExtendedTest.flags.wfrp2e.reloading = weapon.id
      ...
      if (weapon.actor.type == "vehicle") {
        ...
        reloadExtendedTest.flags.wfrp2e.vehicle = vehicleSpeaker
      }
      ...
      this.createEmbeddedDocuments("Item", [reloadExtendedTest]).then(item => {
        ...
        weapon.update({ "flags.wfrp2e.reloading": item[0].id })
      })
    }
  ...
  async addCondition(effect, value = 1) {
    if (typeof (effect) === "string")
      effect = duplicate(game.wfrp2e.config.statusEffects.find(e => e.id == effect))
    if (!effect)
    ...
    if (existing && !existing.isNumberedCondition)
      ...
      return existing.setFlag("wfrp2e", "value", existing.conditionValue + value)
    }
    else if (!existing) {
      if (game.combat && (effect.id == "blinded" || effect.id == "deafened"))
        effect.flags.wfrp2e.roundReceived = game.combat.round
      effect.label = game.i18n.localize(effect.label);

      if (Number.isNumeric(effect.flags.wfrp2e.value))
        effect.flags.wfrp2e.value = value;
  ...
  async removeCondition(effect, value = 1) {
    ...
    if (existing && !existing.isNumberedCondition) {
      ...
      await existing.setFlag("wfrp2e", "value", existing.conditionValue - value);
      ...
      if (existing.conditionValue == 0 && (effect.id == "bleeding" || effect.id == "poisoned" || effect.id == "broken" || effect.id == "stunned")) {
        if (!game.settings.get("wfrp2e", "mooConditions") || !effect.id == "broken") // Homebrew rule prevents broken from causing fatigue
          await this.addCondition("fatigued")
  ...
  applyFear(value, name = undefined) {
    value = value || 0
    let fear = duplicate(game.wfrp2e.config.systemItems.fear)
    fear.data.SL.target = value;

    if (name)
      fear.effects[0].flags.wfrp2e.fearName = name
  ...
  applyTerror(value, name = undefined) {
    value = value || 1
    let terror = duplicate(game.wfrp2e.config.systemItems.terror)
    terror.flags.wfrp2e.terrorValue = value
    game.wfrp2e.utility.applyOneTimeEffect(terror, this)
  }
  ...
  populateEffect(effectId, item, test) {
    ...
    let script = getProperty(effect, "flags.wfrp2e.script")
    if (test && script) {
      let regex = /{{(.+?)}}/g
      ...
      setProperty(effect, "flags.wfrp2e.script", script)
    }
  ...
  addSystemEffect(key) {
    let systemEffects = game.wfrp2e.utility.getSystemEffects()
  ...
  // @@@@@@@@@@@ COMPUTED GETTERS @@@@@@@@@
  get Species() {
    let species = game.wfrp2e.config.species[this.details.species.value] || this.details.species.value
    if (this.details.species.subspecies && game.wfrp2e.config.subspecies[this.details.species.value] && game.wfrp2e.config.subspecies[this.details.species.value][this.details.species.subspecies])
      species += ` (${game.wfrp2e.config.subspecies[this.details.species.value][this.details.species.subspecies].name})`
  ...
  get sizeNum() {
    return game.wfrp2e.config.actorSizeNums[this.details.size.value]
  }
    ...
    return this.data.data.passengers.map(p => {
      ...
      if (actor)
        return {
          ...
          count: p.count,
          enc: game.wfrp2e.config.actorSizeEncumbrance[actor.details.size.value] * p.count
        }
    })
  }
