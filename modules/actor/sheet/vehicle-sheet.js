 ...
 *
 */
export default class ActorSheetWfrp2eVehicle extends ActorSheetWfrp2e {
  static get defaultOptions() {
    const options = super.defaultOptions;
    mergeObject(options,
      {
        classes: options.classes.concat(["wfrp2e", "actor", "vehicle-sheet"]),
        width: 610,
        height: 740,
  ...
  get template() {
    ...
    return "systems/wfrp2e/templates/actors/vehicle/vehicle-sheet.html";
  }
  ...
  async passengerSelect(dialogMessage = game.i18n.localize("DIALOG.ActorSelection")) {
    return new Promise((resolve, reject) => {
      renderTemplate("systems/wfrp2e/templates/dialog/vehicle-weapon.html", { dialogMessage, actors: this.actor.passengers.map(p => p.actor) }).then(dlg => {
        new Dialog({
          content: dlg,
  ...
  async _onRoleSkillClick(ev) {
    ...
    if (ev.button == 0) {
      ...
      if (!skill) {
        let char = game.wfrp2e.utility.findKey(test, game.wfrp2e.config.characteristics)
        if (!char)
...
// Register NPC Sheet
Actors.registerSheet("wfrp2e", ActorSheetWfrp2eVehicle,
  {
    types: ["vehicle"],
    makeDefault: true
  });
