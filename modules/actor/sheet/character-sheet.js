...
import ActorSheetWfrp2e from "./actor-sheet.js";
 ...
 *
 */
export default class ActorSheetWfrp2eCharacter extends ActorSheetWfrp2e {
  ...
  get template() {
    ...
    return "systems/wfrp2e/templates/actors/character/character-sheet.html";
  ...
  addCharacterData(sheetData) {
    ...
    for (let career of sheetData.actor.getItemTypes("career")) {
      if (career.current.value) {
        ...
        if (!sheetData.actor.details.status.value) // backwards compatible with moving this to the career change handler
          sheetData.career.status = game.wfrp2e.config.statusTiers[career.status.tier] + " " + career.status.standing;
    ...
    sheetData.data.details.experience.canEdit = game.user.isGM || game.settings.get("wfrp2e", "playerExperienceEditing")
  }
  ...
  activateListeners(html) {
    ...
    html.find(".add-career").click(ev => {new game.wfrp2e.apps.CareerSelector(this.actor).render(true)})
  ...
  async _onAdvancementClick(ev) {
    ...
    // Characteristics
      ...
      if (ev.button == 0) {
        ...
        try {
          WFRP_Utility.checkValidAdvancement(data.details.experience.total, data.details.experience.spent + cost, game.i18n.localize("ACTOR.ErrorImprove"), game.wfrp2e.config.characteristics[characteristic]);
          data.characteristics[characteristic].advances++;
          data.details.experience.spent = Number(data.details.experience.spent) + cost;

          let expLog = this.actor._addToExpLog(cost, game.wfrp2e.config.characteristics[characteristic], data.details.experience.spent)
          ui.notifications.notify(game.i18n.format("ACTOR.SpentExp", {amount : cost, reason : game.wfrp2e.config.characteristics[characteristic]}))
          data.details.experience.log = expLog
        ...
        let expLog = this.actor._addToExpLog(-1 * cost, game.wfrp2e.config.characteristics[characteristic], data.details.experience.spent)
        ui.notifications.notify(game.i18n.format("ACTOR.SpentExp", {amount : -1 * cost, reason : game.wfrp2e.config.characteristics[characteristic]}))
        data.details.experience.log = expLog
