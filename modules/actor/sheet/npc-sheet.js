 ...
 *
 */
export default class ActorSheetWfrp2eNPC extends ActorSheetWfrp2e {
  ...
  get template() {
    ...
    return "systems/wfrp2e/templates/actors/npc/npc-sheet.html";
  }
  ...
  async _onNpcIncomeClick(event) {
    ...
    let dieAmount = game.wfrp2e.config.earningValues[WFRP_Utility.findKey(status[0], game.wfrp2e.config.statusTiers)][0] // b, s, or g maps to 2d10, 1d10, or 1 respectively (takes the first letter)
    dieAmount = Number(dieAmount) * status[1];     // Multilpy that first letter by your standing (Brass 4 = 8d10 pennies)
    let moneyEarned;
    if (WFRP_Utility.findKey(status[0], game.wfrp2e.config.statusTiers) != "g") // Don't roll for gold, just use standing value
    {
    ...
    let paystring
    switch (WFRP_Utility.findKey(status[0], game.wfrp2e.config.statusTiers)) {
      case "b":
    ...
    let money = MarketWfrp2e.creditCommand(paystring, this.actor, { suppressMessage: true })
        ...
        buttons: {
          yes: {
            ...
            callback: async () => {
              ...
              await this.actor.update({ "data.details.status.value": game.wfrp2e.config.statusTiers[careerItem.status.tier] + " " + careerItem.status.standing })
            }
          },
          no: {
