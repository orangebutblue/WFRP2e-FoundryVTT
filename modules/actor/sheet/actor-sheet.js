...
import ActiveEffectWfrp2e from "../../system/effect-wfrp2e.js"
 ...
 */
export default class ActorSheetWfrp2e extends ActorSheet {
  ...
  getData() {
    ...
    if (this.actor.type != "vehicle") {
      sheetData.effects.system = game.wfrp2e.utility.getSystemEffects();
    }
    return sheetData;
  }
  ...
  constructInventory(sheetData) {
    ...
    const categories = {
      ...
      foodAndDrink: {
        label: game.i18n.localize("WFRP2E.TrappingType.FoodDrink"),
      ...
      cargo: {
        label: game.i18n.localize("WFRP2E.TrappingType.Cargo"),
  ...
  addConditionData(sheetData) {
    let conditions = duplicate(game.wfrp2e.config.statusEffects).map(e => new ActiveEffectWfrp2e(e));
    ...
    for (let condition of conditions) {
      ...
      if (owned) {
        ...
        condition.data.flags.wfrp2e.value = owned.conditionValue;
      }
      else if (condition.isNumberedCondition) {
        condition.data.flags.wfrp2e.value = 0
      }
    }
  ...
  formatArmourSection(sheetData) {
    ...
    for (let loc in AP) {
      if (loc == "shield")
        continue
      let table = game.wfrp2e.tables.findTable(sheetData.data.details.hitLocationTable.value)
      if (table)
      {
        let result  = table.data.results.find(r => r.getFlag("wfrp2e", "loc") == loc)
        if (result)
          AP[loc].label = game.i18n.localize(result.data.text)
        else
          AP[loc].show = false;
      }
      else if (game.wfrp2e.config.locations[loc]) // fallback implementation
      {
        AP[loc].label = game.i18n.localize(game.wfrp2e.config.locations[loc])
      }
    }
  }
 ...
 */
  spellDialog(spell, options = {}) {
    ...
    if (spell.lore.value == "petty")
      ...
      renderTemplate("systems/wfrp2e/templates/dialog/cast-channel-dialog.html").then(dlg => {
        new Dialog({
  ...
  activateListeners(html) {
    ...
    html.find("#configure-actor").click(ev => {
      new game.wfrp2e.apps.ActorSettings(this.actor).render(true);
    })
    ...
    html.on("drop", ".mount-drop", ev => {
      ...
      if (game.wfrp2e.config.actorSizeNums[mount.details.size.value] < game.wfrp2e.config.actorSizeNums[this.actor.details.size.value])
  ...
  async _onUnarmedClick(ev) {
    ev.preventDefault();
    let unarmed = game.wfrp2e.config.systemItems.unarmed
  ...
  async _onImprovisedClick(ev) {
    ev.preventDefault();
    let improv = game.wfrp2e.config.systemItems.improv;
  ...
  async _onStompClick(ev) {
    ev.preventDefault();
    let stomp = game.wfrp2e.config.systemItems.stomp;
  ...
  _onSkillSwitch(ev) {
    this.actor.setFlag("wfrp2e", "showExtendedTests", !getProperty(this.actor, "data.flags.wfrp2e.showExtendedTests"))
    this.render(true)
  }
  ...
  _onArmourTotalClick(ev) {
    ...
    for (let armourTrait of armourTraits) {
      ...
      if (armourTrait && !getProperty(armourTrait, "flags.wfrp2e.APdamage")) setProperty(armourTrait, "flags.wfrp2e.APdamage", { head: 0, body: 0, lArm: 0, rArm: 0, lLeg: 0, rLeg: 0 })
      if (armourTrait) {
        if (ev.button == 0) {
          if (armourTrait.flags.wfrp2e.APdamage[location] != 0) {
            armourTrait.flags.wfrp2e.APdamage[location]--;
            usedTrait = true
          }
        }
        if (ev.button == 2) {
          // If AP Damage at location is maxed, go to the next iteration
          if (armourTrait.flags.wfrp2e.APdamage[location] == Number(armourTrait.data.specification.value)) { continue }
          // Else, damage that location
          if (armourTrait.flags.wfrp2e.APdamage[location] != Number(armourTrait.data.specification.value)) {
            armourTrait.flags.wfrp2e.APdamage[location]++;
            usedTrait = true
          }
        }
  ...
  _onItemDelete(ev) {
    ...
    if (this.actor.items.get(itemId).name == "Boo") {
      AudioHelper.play({ src: `${game.settings.get("wfrp2e", "soundPath")}squeek.wav` }, false)
      return
    }
    renderTemplate('systems/wfrp2e/templates/dialog/delete-item-dialog.html').then(html => {
      new Dialog({
  ...
  _onItemToggle(ev) {
    ...
    if (item.type == "armour") {
      ...
      if (game.settings.get("wfrp2e", "limitEquippedWeapons") && this.actor.type != "vehicle")
  ...
  _onConditionToggle(ev) {
    let condKey = $(ev.currentTarget).parents(".sheet-condition").attr("data-cond-id")
    if (game.wfrp2e.config.statusEffects.find(e => e.id == condKey).flags.wfrp2e.value == null) {
  ...
  async _onSpeciesEdit(ev) {
    ...
    let speciesKey = WFRP_Utility.findKey(species, game.wfrp2e.config.species) || species
    let subspeciesKey = ""
    if (subspecies) {
      for (let sub in game.wfrp2e.config.subspecies[speciesKey]) {
        if (game.wfrp2e.config.subspecies[speciesKey][sub].name == subspecies) subspeciesKey = sub
      }
      if (!subspeciesKey)
  ...
  _onConditionClicked(ev) {
    ...
    let li = $(ev.currentTarget).parents(".sheet-condition"),
      ...
      condkey = li.attr("data-cond-id"), expandData = TextEditor.enrichHTML(`<h2>${game.wfrp2e.config.conditions[condkey]}</h2>` + game.wfrp2e.config.conditionDescriptions[condkey])
    ...
    if (elementToAddTo.hasClass("expanded")) {
      ...
      if (game.wfrp2e.config.conditionScripts[condkey] && this.actor.hasCondition(condkey)) {
        let button = $(`<br><br><a class="condition-script">${game.i18n.format("CONDITION.Apply", { condition: game.wfrp2e.config.conditions[condkey] })}</a>`)
        div.append(button)
      }
      ...
      div.on("click", ".condition-script", async ev => {
        ...
        ChatMessage.create(await game.wfrp2e.config.conditionScripts[condkey](this.actor))
      })
    }
  ...
  _onMoneyIconClicked(ev) {
    ...
    let newMoney = MarketWfrp2e.consolidateMoney(money.map(i => i.toObject()));
  ...
  _onEffectCreate(ev) {
    ...
    if (type == "applied") {
      effectData["flags.wfrp2e.effectApplication"] = "apply"
    }
  ...
  _onInvokeClick(ev) {
    let id = $(ev.currentTarget).parents(".item").attr("data-item-id");
    game.wfrp2e.utility.invokeEffect(this.actor, id)
  }
  ...
  _onDropCharGen(dragData) {
    ...
    if (dragData.generationType == "attributes") // Characteristsics, movement, metacurrency, etc.
    {
      ...
      for (let c in game.wfrp2e.config.characteristics) {
  ...
  _dropdownListeners(html) {
    ...
    html.on("click", ".apply-effect", async ev => {
      ...
      if (effect.flags.wfrp2e?.reduceQuantity)
      {
      ...
      if ((item.range && item.range.value.toLowerCase() == game.i18n.localize("You").toLowerCase()) && (item.target && item.target.value.toLowerCase() == game.i18n.localize("You").toLowerCase()))
        game.wfrp2e.utility.applyEffectToTarget(effect, [{ actor: this.actor }]) // Apply to caster (self)
      else
        game.wfrp2e.utility.applyEffectToTarget(effect)
    })
    ...
    html.on("click", ".invoke-effect", async ev => {
      ...
      game.wfrp2e.utility.invokeEffect(this.actor, effectId, itemId)
    })
  ...
  _expandProperty(ev) {
    ...
    let li = $(ev.currentTarget).parents(".item"),
      ...
      propertyDescr = Object.assign(duplicate(game.wfrp2e.config.qualityDescriptions), game.wfrp2e.config.flawDescriptions); // Property descriptions
  ...
  _expandInfo(ev) {
    ...
    if (classes.hasClass("weapon-range")) {
      ...
      expansionText =
        `<a class="range-click" data-range="${item.range.bands[`${game.i18n.localize("Point Blank")}`].modifier}">${item.range.bands[`${game.i18n.localize("Point Blank")}`].range[0]} ${game.i18n.localize("yds")} - ${item.range.bands[`${game.i18n.localize("Point Blank")}`].range[1]} ${game.i18n.localize("yds")}: ${game.wfrp2e.config.difficultyLabels[game.wfrp2e.config.rangeModifiers["Point Blank"]]}</a><br>
          <a class="range-click" data-range="${item.range.bands[`${game.i18n.localize("Short Range")}`].modifier}">${item.range.bands[`${game.i18n.localize("Short Range")}`].range[0]} ${game.i18n.localize("yds")} - ${item.range.bands[`${game.i18n.localize("Short Range")}`].range[1]} ${game.i18n.localize("yds")}: ${game.wfrp2e.config.difficultyLabels[game.wfrp2e.config.rangeModifiers["Short Range"]]}</a><br>
          <a class="range-click" data-range="${item.range.bands[`${game.i18n.localize("Normal")}`].modifier}">${item.range.bands[`${game.i18n.localize("Normal")}`].range[0]} ${game.i18n.localize("yds")} - ${item.range.bands[`${game.i18n.localize("Normal")}`].range[1]} ${game.i18n.localize("yds")}: ${game.wfrp2e.config.difficultyLabels[game.wfrp2e.config.rangeModifiers["Normal"]]}</a><br>
          <a class="range-click" data-range="${item.range.bands[`${game.i18n.localize("Long Range")}`].modifier}">${item.range.bands[`${game.i18n.localize("Long Range")}`].range[0]} ${game.i18n.localize("yds")} - ${item.range.bands[`${game.i18n.localize("Long Range")}`].range[1]} ${game.i18n.localize("yds")}: ${game.wfrp2e.config.difficultyLabels[game.wfrp2e.config.rangeModifiers["Long Range"]]}</a><br>
          <a class="range-click" data-range="${item.range.bands[`${game.i18n.localize("Extreme")}`].modifier}">${item.range.bands[`${game.i18n.localize("Extreme")}`].range[0]} ${game.i18n.localize("yds")} - ${item.range.bands[`${game.i18n.localize("Extreme")}`].range[1]} ${game.i18n.localize("yds")}: ${game.wfrp2e.config.difficultyLabels[game.wfrp2e.config.rangeModifiers["Extreme"]]}</a><br>
          `
    ...
    // Expand the weapon's group description
      ...
      let weaponGroupKey = "";
      weaponGroupKey = WFRP_Utility.findKey(weaponGroup, game.wfrp2e.config.weaponGroups);
      expansionText = game.wfrp2e.config.weaponGroupDescriptions[weaponGroupKey];
    }
    // Expand the weapon's reach description
    else if (classes.hasClass("weapon-reach")) {
      let reach = ev.target.text;
      let reachKey;
      reachKey = WFRP_Utility.findKey(reach, game.wfrp2e.config.weaponReaches);
      expansionText = game.wfrp2e.config.reachDescription[reachKey];
    }
