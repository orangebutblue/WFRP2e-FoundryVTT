...
import ActorWfrp2e from "../actor/actor-wfrp2e.js";
    ...
    options.push(
      {
        ...
        callback: li => {
          ...
          if (li.find(".dice-roll").length) {
            ...
            let updateMsg = opposedTest.defenderTest.actor.applyDamage(opposedTest, game.wfrp2e.config.DAMAGE_TYPE.NORMAL)
      ...
      },
      {
        ...
        callback: li => {
          if (li.find(".dice-roll").length) {
            let amount = li.find('.dice-total').text();
            game.user.targets.forEach(t => t.actor.applyBasicDamage(amount, { damageType: game.wfrp2e.config.DAMAGE_TYPE.IGNORE_AP }))
          }
          else {
            let message = game.messages.get(li.attr("data-message-id"))
            let opposedTest = message.getOpposedTest();
            ...
            let updateMsg = opposedTest.defenderTest.actor.applyDamage(opposedTest, game.wfrp2e.config.DAMAGE_TYPE.IGNORE_AP)
            OpposedWFRP.updateOpposedMessage(updateMsg, message.id);
          }
        }
      },
      {
        ...
        callback: li => {
          if (li.find(".dice-roll").length) {
            let amount = li.find('.dice-total').text();
            game.user.targets.forEach(t => t.actor.applyBasicDamage(amount, { damageType: game.wfrp2e.config.DAMAGE_TYPE.IGNORE_TB }))
          }
          else {
            let message = game.messages.get(li.attr("data-message-id"))
            let opposedTest = message.getOpposedTest();
            ...
            let updateMsg = opposedTest.defenderTest.actor.applyDamage(opposedTest, game.wfrp2e.config.DAMAGE_TYPE.IGNORE_TB)
            OpposedWFRP.updateOpposedMessage(updateMsg, message.id);
          }
        }
      },
      {
        ...
        callback: li => {
          if (li.find(".dice-roll").length) {
            let amount = li.find('.dice-total').text();
            game.user.targets.forEach(t => t.actor.applyBasicDamage(amount, { damageType: game.wfrp2e.config.DAMAGE_TYPE.IGNORE_ALL }))
          }
          else {
            let message = game.messages.get(li.attr("data-message-id"))
            let opposedTest = message.getOpposedTest();
            ...
            let updateMsg = opposedTest.defenderTest.actor.applyDamage(opposedTest, game.wfrp2e.config.DAMAGE_TYPE.IGNORE_ALL)
            OpposedWFRP.updateOpposedMessage(updateMsg, message.id);
          }
        }
      },
      ...
      },
      {
        ...
        callback: li => {
          ...
          test.opposedMessages.forEach(message => {
            ...
            let updateMsg = opposedTest.defender.applyDamage(opposedTest.resultMessage.getOpposedTest(), game.wfrp2e.config.DAMAGE_TYPE.NORMAL)
            OpposedWFRP.updateOpposedMessage(updateMsg, message.id);
          })
        }
      }
