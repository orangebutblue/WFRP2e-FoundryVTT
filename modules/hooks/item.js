    ...
    try {
      ...
      if (item.type == "critical") {
        ...
        if (game.combat && game.user.isGM) {
          let minorInfections = game.combat.getFlag("wfrp2e", "minorInfections") || []
          minorInfections.push(item.actor.name)
          game.combat.setFlag("wfrp2e", "minorInfections", null).then(c => game.combat.setFlag("wfrp2e", "minorInfections", minorInfections))
        }
      }
    }
    catch (error) {
