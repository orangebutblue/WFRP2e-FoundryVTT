...
export default function() {
  ...
  Hooks.on("renderTokenHUD", async (obj, html) => {
    for (let condition of html.find("img.effect-control")) {
      condition.title = game.wfrp2e.config.conditions[condition.dataset["statusId"]]
  ...
  Hooks.on("createToken", async (token) => {
    ...
    if(game.user.isUniqueGM) // Prevents multiple mount tokens
    {
      ...
      if (token.actor.isMounted && canvas.scene.id == scene.id)
      {
        ...
        await token.update({"flags.wfrp2e.mount" : mountToken.id }) // place mount id in token so when it moves, the mount moves (see updateToken)
  ...
  Hooks.on("updateToken", (token, updateData, options) => {
      ...
      {
        if (hasProperty(token, "data.flags.wfrp2e.mount") && (updateData.x || updateData.y) && scene.id == canvas.scene.id)
        {
          if (canvas.tokens.get(token.id).actor.isMounted)
          {
            let mountId = token.getFlag("wfrp2e", "mount")
      ...
      if (hasProperty(updateData, "flags.wfrp2e.mask") && token.actorLink == true)
      {
        game.actors.get(token.actorId).update({"token.flags.wfrp2e.mask" : getProperty(updateData, "flags.wfrp2e.mask") })
      }
  })
  ...
  Hooks.on('renderTokenHUD', (hud, html) => {
    ...
    if (canvas.tokens.controlled.length == 2)// && canvas.tokens.controlled[0].actor.details.size.value != canvas.tokens.controlled[1].actor.details.size.value)
    {
      ...
      button.mousedown(event => {
        ...
        if (game.wfrp2e.config.actorSizeNums[mounter.actor.details.size.value] > game.wfrp2e.config.actorSizeNums[mountee.actor.details.size.value])
        {
          let temp = mountee;
          mountee = mounter
          mounter = temp
        ...
        canvas.scene.updateEmbeddedDocuments("Token", [{ "flags.wfrp2e.mount": mountee.id, _id: mounter.id }, { _id: mounter.id, x: mountee.data.x, y: mountee.data.y }])
