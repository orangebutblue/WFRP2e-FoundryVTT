...
import ChatWFRP from "../system/chat-wfrp2e.js";
...
export default function() {
  ...
  // Activate chat listeners defined in chat-wfrp2e.js
  ...
  Hooks.on("chatMessage", (html, content, msg) => {
     ...
     * @returns {{amount: string, option:  game.wfrp2e.config.creditOptions}}
     */
    function extractAmountAndOptionFromCommandLine(commands) {
      ...
      if (isAnAmount) {
        ...
        optionInCommandLine =  game.wfrp2e.config.creditOptions.SPLIT;
      } else {
     ...
     * @returns { game.wfrp2e.config.creditOptions} an option
     */
    function getOption(optionInCommandLine) {
      return (typeof optionInCommandLine == "undefined") ?  game.wfrp2e.config.creditOptions.SPLIT : optionInCommandLine;
    }
    ...
    if (command === "/table") {
      ...
      {
        game.wfrp2e.tables.formatChatRoll("menu").then(text => {
          if (!text)
            return
          msg.content = text
        ...
        game.wfrp2e.tables.formatChatRoll(commands[1], { modifier: modifier }, column).then(text => {
          if (!text)
            return
          msg.content = text
          ChatMessage.create(msg);
    ...
    // Lookup a condition
      ...
      let closest = WFRP_Utility.matchClosest( game.wfrp2e.config.conditions, conditionInput);
      if (! game.wfrp2e.config.conditionDescriptions) {
        ui.notifications.error("No content found")
        return false
      }
      let description =  game.wfrp2e.config.conditionDescriptions[closest];
      let name =  game.wfrp2e.config.conditions[closest];
    ...
    else if (command === "/prop")
    {
      ...
      let allProperties = game.wfrp2e.utility.allProperties();
      let closest = WFRP_Utility.matchClosest( game.wfrp2e.utility.allProperties(), propertyInput);

      let description = game.wfrp2e.config.qualityDescriptions[closest] || game.wfrp2e.config.flawDescriptions[closest];
    ...
    // Pay commands
      ...
      if (!game.user.isGM) {
        ...
        let money = MarketWfrp2e.payCommand(amount, actor);
        if (money)
    ...
    // Travel commands
      ...
      TravelDistanceWfrp2e.displayTravelDistance( commands[1], commands[2] );
      return false;
    }
    ...
    //Help commands
      ...
      let link = game.i18n.format("CHAT.CommandLine.Help.Link", { link: "https://github.com/moo-man/WFRP2E-FoundryVTT/wiki" })

      renderTemplate("systems/wfrp2e/templates/chat/help/chat-help-command.html", {
    ...
    if (postedItem) {
      ...
      postedItem.addEventListener('dragstart', ev => {
        ...
        if (game.user.isGM)
        {
          ...
          renderTemplate("systems/wfrp2e/templates/chat/post-item.html", recreateData).then(html => {
