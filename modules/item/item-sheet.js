...
import ItemWfrp2e from "./item-wfrp2e.js";
import WFRP_Utility from "../system/utility-wfrp2e.js";



export default class ItemSheetWfrp2e extends ItemSheet {
   ...
   *
   * Example: A weapon sheet needs all different weapon types to list in the weaponGroup dropdown (`data['weaponGroups'] =  game.wfrp2e.config.weaponGroups;`)
   */
  getData() {
    ...
    if (this.item.type == "spell") {
      if (game.wfrp2e.config.magicLores[this.item.lore.value]) {
        data["loreValue"] = game.wfrp2e.config.magicLores[this.item.lore.value]
      }
      else {
    ...
    //@HOUSE
    if (this.item.type == "weapon" && game.settings.get("wfrp2e", "mooRangeBands"))
    {
    ...
    //@/HOUSE
      ...
      let characteristicList = duplicate(game.wfrp2e.config.characteristicsAbbrev);
      for (let char in characteristicList) {
        if (data.data.characteristics.includes(char))
          characteristicList[char] = {
            abrev: game.wfrp2e.config.characteristicsAbbrev[char],
            checked: true
          };
        else
          characteristicList[char] = {
      ...
      data.cargoTypes = game.wfrp2e.config.trade.cargoTypes
      data.qualities = game.wfrp2e.config.trade.qualities
      data["dotrActive"] = (game.modules.get("wfrp2e-dotr") && game.modules.get("wfrp2e-dotr").active)
    }
  ...
  addConditionData(data) {
    ...
    data.conditions = duplicate(game.wfrp2e.config.statusEffects).filter(i => i.id != "fear" && i.id != "grappling");
    ...
    for (let condition of data.conditions) {
      ...
      if (existing) {
        condition.value = existing.flags.wfrp2e.value
        condition.existing = true;
      }
      else condition.value = 0;

      if (condition.flags.wfrp2e.value == null)
  ...
  activateListeners(html) {
    ...
    html.find(".edit-item-properties").click(ev => {
      new game.wfrp2e.apps.ItemProperties(this.item).render(true)
    })
    html.find(".cargo-sell").click(ev => {
      game.wfrp2e.apps.Wfrp2eTradeManager.processTradeSell(this.item)
    })
  ...
  async _onLoreChange(event) {
    ...
    {
      let loreEffects = this.item.effects.filter(i => i.flags.wfrp2e.lore)
      ...
      for (let lore in game.wfrp2e.config.magicLores) {
        // If lore value matches config, use that (Update the actor with the "key" value)
        if (inputLore == game.wfrp2e.config.magicLores[lore]) {
          this.item.createEmbeddedDocuments("ActiveEffect", [game.wfrp2e.config.loreEffects[lore]])
  ...
  async _onSymptomChange(event) {
    ...
    let symptomKeys = symtomNames.map(s => game.wfrp2e.utility.findKey(s, game.wfrp2e.config.symptoms))
    ...
    let symptomEffects = symptomKeys.map((s, i) => {
      if (game.wfrp2e.config.symptomEffects[s]) {
        let effect = duplicate(game.wfrp2e.config.symptomEffects[s])
    ...
    let effects = this.item.effects.map(i => i.toObject()).filter(e => getProperty(e, "flags.wfrp2e.symptom"))
