...
export default class ItemWfrp2e extends Item {
  /**
  ...
  async _preCreate(data, options, user) {
    ...
    if (!data.img)
      this.data.update({ img: "systems/wfrp2e/icons/blank.png" });
    ...
    if (this.isOwned) {
      ...
      if (this.effects.size) {
        ...
        immediateEffects.forEach(effect => {
          game.wfrp2e.utility.applyOneTimeEffect(effect, this.actor)
  ...
  prepareOwnedData() {
    ...
    try {
      ...
      game.wfrp2e.utility.log(`Something went wrong when preparing actor item ${this.name}: ${e}`)
    }
  ...
  prepareOwnedWeapon() {
    ...
    if (game.settings.get("wfrp2e", "mooQualities")) {
      game.wfrp2e.utility.logHomebrew("mooQualities")
      ...
      if (momentum?.value && this.actor.status.advantage.value > 0) {
        ...
        this._addProperties({ qualities: game.wfrp2e.utility.propertyStringToObject(qualityString, game.wfrp2e.utility.allProperties()), flaws: {} })
  ...
  _mutationExpandData() {
    let data = this.toObject().data
    data.properties = [];
    data.properties.push(game.wfrp2e.config.mutationTypes[this.mutationType.value]);
  ...
  _careerExpandData() {
    ...
    data.properties.push(game.wfrp2e.config.statusTiers[this.status.tier] + " " + this.status.standing);
    data.properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.characteristics.map(i => i = " " + game.wfrp2e.config.characteristicsAbbrev[i])}`);
  ...
  _weaponExpandData() {
    ...
    if (this.weaponGroup.value)
      properties.push(game.wfrp2e.config.weaponGroups[this.weaponGroup.value]);
    if (this.range.value)
    ...
    if (this.reach.value)
      properties.push(`${game.i18n.localize("Reach")}: ${game.wfrp2e.config.weaponReaches[this.reach.value] + " - " + game.wfrp2e.config.reachDescription[this.reach.value]}`);
  ...
  _armourExpandData() {
    let data = this.toObject().data
    let properties = [];
    properties.push(game.wfrp2e.config.armorTypes[this.armorType.value]);
    let itemProperties = this.Qualities.concat(this.Flaws)
    for (let prop of itemProperties)
      properties.push("<a class ='item-property'>" + prop + "</a>")
  ...
  _ammunitionExpandData() {
    let data = this.toObject().data
    let properties = [];
    properties.push(game.wfrp2e.config.ammunitionGroups[this.ammunitionType.value])
  ...
  _vehicleModExpandData() {
    let data = this.toObject().data
    data.properties = [game.wfrp2e.config.modTypes[this.modType.value]];
    return data;
  }
  ...
  _cargoExpandData() {
    ...
    if (game.wfrp2e.config.trade.cargoTypes)
      data.properties.push(`<b>${game.i18n.localize("ITEM.CargoType")}</b>: ${game.wfrp2e.config.trade.cargoTypes[this.cargoType.value]}`)

    if (game.wfrp2e.config.trade.qualities && (this.cargoType.value == "wine" || this.cargoType.value == "brandy"))
      data.properties.push(`<b>${game.i18n.localize("ITEM.CargoQuality")}</b>: ${game.wfrp2e.config.trade.qualities[this.quality.value]}`)
  ...
  _trappingChatData() {
    let properties = [
      `<b>${game.i18n.localize("ITEM.TrappingType")}</b>: ${game.wfrp2e.config.trappingCategories[this.trappingType.value]}`,
      ...
      `<b>${game.i18n.localize("Availability")}</b>: ${game.wfrp2e.config.availability[this.availability.value] || "-"}`
    ]
  ...
  _mutationChatData() {
    let properties = [
      `<b>${game.i18n.localize("ITEM.MutationType")}</b>: ${game.wfrp2e.config.mutationTypes[this.mutationType.value]}`,
    ];
    if (this.modifier.value)
  ...
  _talentChatData() {
    let properties = [];
    properties.push(`<b>${game.i18n.localize("Max")}: </b> ${game.wfrp2e.config.talentMax[this.max.value]}`);
    if (this.tests.value)
  ...
  _careerChatData() {
    ...
    properties.push(`<b>${game.i18n.localize("Status")}</b>: ${game.wfrp2e.config.statusTiers[this.status.tier] + " " + this.status.standing}`);
    properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.characteristics.map(i => i = " " + game.wfrp2e.config.characteristicsAbbrev[i])}`);
  ...
  _spellChatData() {
    let properties = [];
    if (game.wfrp2e.config.magicLores[this.lore.value])
      properties.push(`<b>${game.i18n.localize("Lore")}</b>: ${game.wfrp2e.config.magicLores[this.lore.value]}`);
    else
  ...
  _weaponChatData() {
    ...
    if (this.weaponGroup.value)
      properties.push(`<b>Group</b>: ${game.wfrp2e.config.weaponGroups[this.weaponGroup.value]}`);
    if (this.range.value)
      properties.push(`<b>${game.i18n.localize("Range")}</b>: ${this.range.value}`);
    if (this.damage.value)
      properties.push(`<b>${game.i18n.localize("Damage")}</b>: ${this.damage.value}`);
    ...
    if (this.reach.value)
      properties.push(`<b>${game.i18n.localize("Reach")}</b>: ${game.wfrp2e.config.weaponReaches[this.reach.value] + " - " + game.wfrp2e.config.reachDescription[this.reach.value]}`);
    if (this.damageToItem.value)
      properties.push(`<b>${game.i18n.localize("ITEM.WeaponDamaged")} ${this.damageToItem.value} points</b>`)
    if (this.damageToItem.shield)
      properties.push(`${game.i18n.localize("ITEM.ShieldDamaged")} ${this.damageToItem.shield} points`)
  ...
  _armourChatData() {
    ...
    if (this.armorType.value)
      properties.push(`<b>${game.i18n.localize("ITEM.ArmourType")}</b>: ${game.wfrp2e.config.armorTypes[this.armorType.value]}`);
    if (this.penalty.value)
      properties.push(`<b>${game.i18n.localize("Penalty")}</b>: ${this.penalty.value}`);
    ...
    for (let loc in game.wfrp2e.config.locations)
      if (this.maxAP[loc])
        properties.push(`<b>${game.wfrp2e.config.locations[loc]} AP</b>: ${this.currentAP[loc]}/${this.maxAP[loc]}`);
  ...
  _ammunitionChatData() {
    ...
    properties.push(`<b>${game.i18n.localize("ITEM.AmmunitionType")}:</b> ${game.wfrp2e.config.ammunitionGroups[this.ammunitionType.value]}`)
  ...
  _vehicleModChatData() {
    let properties = [
      `<b>${game.i18n.localize("VEHICLE.ModType")}</b>: ${game.wfrp2e.config.modTypes[this.modType.value]}`,
      `<b>${game.i18n.localize("Price")}</b>: ${this.price.gc || 0} ${game.i18n.localize("MARKET.Abbrev.GC")}, ${this.price.ss || 0} ${game.i18n.localize("MARKET.Abbrev.SS")}, ${this.price.bp || 0} ${game.i18n.localize("MARKET.Abbrev.BP")}`,
      `<b>${game.i18n.localize("Encumbrance")}</b>: ${this.encumbrance.value}`,
    ]
    return properties
  ...
  _cargoChatData() {
    ...
    if (game.wfrp2e.config.trade.cargoTypes)
      properties.push(`<b>${game.i18n.localize("ITEM.CargoType")}</b>: ${game.wfrp2e.config.trade.cargoTypes[this.cargoType.value]}`)

    if (game.wfrp2e.config.trade.qualities && (this.cargoType.value == "wine" || this.cargoType.value == "brandy"))
      properties.push(`<b>${game.i18n.localize("ITEM.CargoQuality")}</b>: ${game.wfrp2e.config.trade.qualities[this.quality.value]}`)
    return properties;
  }
    ...
    try {
      ...
      if (formula != game.i18n.localize("You").toLowerCase() && formula != game.i18n.localize("Special").toLowerCase() && formula != game.i18n.localize("Instant").toLowerCase()) {
        ...
        for (let ch in this.actor.characteristics) {
          ...
          if (formula.includes(game.wfrp2e.config.characteristics[ch].toLowerCase())) {
            // Determine if it's looking for the bonus or the value
            if (formula.includes('bonus'))
              formula = formula.replace(game.wfrp2e.config.characteristics[ch].toLowerCase().concat(" bonus"), this.actor.characteristics[ch].bonus);
            else
              formula = formula.replace(game.wfrp2e.config.characteristics[ch].toLowerCase(), this.actor.characteristics[ch].value);
          }
        }
      }
 ...
 */
  ...
  computeRangeBands() {
    ...
    rangeBands[`${game.i18n.localize("Point Blank")}`] = {
      ...
      modifier: game.wfrp2e.config.difficultyModifiers[game.wfrp2e.config.rangeModifiers["Point Blank"]],
      difficulty: game.wfrp2e.config.rangeModifiers["Point Blank"]
    }
    rangeBands[`${game.i18n.localize("Short Range")}`] = {
      range: [Math.ceil(range / 10) + 1, Math.ceil(range / 2)],
      modifier: game.wfrp2e.config.difficultyModifiers[game.wfrp2e.config.rangeModifiers["Short Range"]],
      difficulty: game.wfrp2e.config.rangeModifiers["Short Range"]
    }
    rangeBands[`${game.i18n.localize("Normal")}`] = {
      range: [Math.ceil(range / 2) + 1, range],
      modifier: game.wfrp2e.config.difficultyModifiers[game.wfrp2e.config.rangeModifiers["Normal"]],
      difficulty: game.wfrp2e.config.rangeModifiers["Normal"]
    }
    rangeBands[`${game.i18n.localize("Long Range")}`] = {
      range: [range + 1, range * 2],
      modifier: game.wfrp2e.config.difficultyModifiers[game.wfrp2e.config.rangeModifiers["Long Range"]],
      difficulty: game.wfrp2e.config.rangeModifiers["Long Range"]
    }
    rangeBands[`${game.i18n.localize("Extreme")}`] = {
      range: [range * 2 + 1, range * 3],
      modifier: game.wfrp2e.config.difficultyModifiers[game.wfrp2e.config.rangeModifiers["Extreme"]],
      difficulty: game.wfrp2e.config.rangeModifiers["Extreme"]
    }

    //@HOUSE
    if (game.settings.get("wfrp2e", "mooRangeBands")) {
      game.wfrp2e.utility.logHomebrew("mooRangeBands")
      if (!this.getFlag("wfrp2e", "optimalRange"))
        game.wfrp2e.utility.log("Warning: No Optimal Range set for " + this.name)

      rangeBands[`${game.i18n.localize("Point Blank")}`].modifier = game.wfrp2e.utility.optimalDifference(this, game.i18n.localize("Point Blank")) * -20 + 20
      delete rangeBands[`${game.i18n.localize("Point Blank")}`].difficulty
      rangeBands[`${game.i18n.localize("Short Range")}`].modifier = game.wfrp2e.utility.optimalDifference(this, game.i18n.localize("Short Range")) * -20 + 20
      delete rangeBands[`${game.i18n.localize("Short Range")}`].difficulty
      rangeBands[`${game.i18n.localize("Normal")}`].modifier = game.wfrp2e.utility.optimalDifference(this, game.i18n.localize("Normal")) * -20 + 20
      delete rangeBands[`${game.i18n.localize("Normal")}`].difficulty
      rangeBands[`${game.i18n.localize("Long Range")}`].modifier = game.wfrp2e.utility.optimalDifference(this, game.i18n.localize("Long Range")) * -20 + 20
      delete rangeBands[`${game.i18n.localize("Long Range")}`].difficulty
      rangeBands[`${game.i18n.localize("Extreme")}`].modifier = game.wfrp2e.utility.optimalDifference(this, game.i18n.localize("Extreme")) * -20 + 20
 ...
 */
  _addSpellDescription() {
    ...
    else if (game.wfrp2e.config.loreEffectDescriptions && game.wfrp2e.config.loreEffectDescriptions[this.lore.value])
      description += `<p>\n\n <b>${game.i18n.localize("SPELL.Lore")}</b> ${game.wfrp2e.config.loreEffectDescriptions[this.lore.value]}<p>`;
  ...
  async addCondition(effect, value = 1) {
    ...
    if (existing && existing.flags.wfrp2e.value == null)
      return existing
    else if (existing) {
      existing = duplicate(existing)
      existing.flags.wfrp2e.value += value;
  ...
  async removeCondition(effect, value = 1) {
    ...
    if (existing && existing.flags.wfrp2e.value == null)
      ...
      if (existing.flags.wfrp2e.value <= 0)
        return this.deleteEmbeddedDocuments("ActiveEffect", [existing._id])
      else
        return this.updateEmbeddedDocuments("ActiveEffect", [existing])
    }
  ...
  // @@@@@@@ BOOLEAN GETTERS @@@@@@
  get isMelee() {
    return this.modeOverride?.value == "melee" || (game.wfrp2e.config.groupToType[this.weaponGroup.value] == "melee" && this.modeOverride?.value != "ranged")
  }

  get isRanged() {
    return this.modeOverride?.value == "ranged" || (game.wfrp2e.config.groupToType[this.weaponGroup.value] == "ranged" && this.modeOverride?.value != "melee")
  }
  ...
  // @@@@@@@ FORMATTED GETTERS @@@@@@
  get WeaponGroup() {
    return game.wfrp2e.config.weaponGroups[this.weaponGroup.value]
  }

  get Reach() {
    return game.wfrp2e.config.weaponReaches[this.reach.value];
  }
  ...
  get attackType() {
    if (this.type == "weapon")
      return this.modeOverride?.value || game.wfrp2e.config.groupToType[this.weaponGroup.value]
  ...
  get reachNum() {
    return game.wfrp2e.config.reachNum[this.reach.value]
  }
  ...
  get reloadingTest() {
    return this.actor.items.get(getProperty(this.data, "flags.wfrp2e.reloading"))
  }
  ...
  get properties() {
    ...
    let properties = {
      qualities: ItemWfrp2e._propertyArrayToObject(this.qualities.value, game.wfrp2e.utility.qualityList()),
      flaws: ItemWfrp2e._propertyArrayToObject(this.flaws.value, game.wfrp2e.utility.flawList()),
      unusedQualities: {}
    }
  ...
  get originalProperties() {
    let properties = {
      qualities: ItemWfrp2e._propertyArrayToObject(this.data._source.data.qualities.value, game.wfrp2e.utility.qualityList()),
      flaws: ItemWfrp2e._propertyArrayToObject(this.data._source.data.flaws.value, game.wfrp2e.utility.flawList()),
      unusedQualities: {}
    }
    return properties;
  }
  ...
  get Damage() {
    ...
    //@HOUSE
    if (game.settings.get("wfrp2e", "mooSizeDamage") && this.actor.sizeNum > 3) {
      if ((this.type == "weapon" && this.damage.value.includes("SB")) || (this.type == "trait" && this.rollable.bonusCharacteristic == "s")) {
        game.wfrp2e.utility.logHomebrew("mooSizeDamage")
  ...
  get trappingCategory() {
    if (this.type == "trapping")
      return game.wfrp2e.config.trappingCategories[this.trappingType.value];
    else
      return game.wfrp2e.config.trappingCategories[this.type];
  }
