...
export default class CombatHelpers {
    ...
    static startTurnChecks(combat) {
        ...
        if (turn) {
            ...
            console.warn("wfrp2e | No actor token found: %o.", combat)
        }
    ...
    static checkInfection(combat) {
        ...
        let minorInfections = combat.getFlag("wfrp2e", "minorInfections") || []
        let content = ""
    ...
    static checkEndRoundConditions(combat) {
        ...
        for (let turn of combat.turns) {
            ...
            for (let cond of endRoundConditions) {
                if (game.wfrp2e.config.conditionScripts[cond.statusId]) {
                    let conditionName = game.i18n.localize(game.wfrp2e.config.conditions[cond.statusId])
                    if (Number.isNumeric(cond.flags.wfrp2e.value))
                        conditionName += ` ${cond.flags.wfrp2e.value}`
                    msgContent = `
              <h2>${conditionName}</h2>
            ...
            for (let cond of conditions) {
                ...
                if (cond.statusId == "deafened" || cond.statusId == "blinded" && Number.isNumeric(cond.flags.wfrp2e.roundReceived)) {
                    if ((combat.round - 1) % 2 == cond.flags.wfrp2e.roundReceived % 2) {
                        turn.actor.removeCondition(cond.statusId)
                        removedConditions.push(
                            game.i18n.format("CHAT.RemovedConditions", {
                                condition: game.i18n.localize(game.wfrp2e.config.conditions[cond.statusId]),
    ...
    static fearReminders(combat) {
        ...
        fearedCombatants.forEach(c => {
            ...
            if (fear.flags.wfrp2e.fearName)
                chatData.content += ` (${fear.flags.wfrp2e.fearName})`
            chatData.content += "<br>"
        })
