export default class Migration {
  ...
  async migrateWorld() {
    ui.notifications.info(`Applying WFRP2E System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`, { permanent: true });
    ...
    for (let i of game.items.contents) {
      try {
        ...
        err.message = `Failed wfrp2e system migration for Item ${i.name}: ${err.message}`;
        console.error(err);
      }
    }
    ...
    for (let a of game.actors.contents) {
      try {
        ...
        err.message = `Failed wfrp2e system migration for Actor ${a.name}: ${err.message}`;
        console.error(err);
      }
    }
    ...
    for (let s of game.scenes.contents) {
      try {
        ...
        err.message = `Failed wfrp2e system migration for Scene ${s.name}: ${err.message}`;
        console.error(err);
      }
    }
    ...
    ui.notifications.info(`wfrp2e System Migration to version ${game.system.data.version} completed!`, { permanent: true });
  };
  ...
  async migrateCompendium(pack) {
    ...
    for (let doc of documents) {
      ...
      // Handle migration failures
        ...
        err.message = `Failed wfrp2e system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
        console.error(err);
      }
    }
 ...
 */
  ...
  cleanActorData(actorData) {
    ...
    // Scrub system flags
    const allowedFlags = CONFIG.wfrp2e.allowedActorFlags.reduce((obj, f) => {
      obj[f] = null;
      return obj;
    }, {});
    if (actorData.flags.wfrp2e) {
      actorData.flags.wfrp2e = filterObject(actorData.flags.wfrp2e, allowedFlags);
    }
  ...
  _migrateItemProperties(item, updateData) {
    ...
    if (typeof item.data.qualities.value == "string") {
      let newQualities = this._migrateProperties(item.data.qualities.value, game.wfrp2e.utility.qualityList())
      updateData["data.qualities.value"] = newQualities
    }
    if (typeof item.data.flaws.value == "string") {
      let newFlaws = this._migrateProperties(item.data.flaws.value, game.wfrp2e.utility.flawList())
  ...
  _migrateEffectScript(effect, updateData) {
    ...
    script = script.replaceAll("item.data.APdamage", "item.getFlag('wfrp2e', 'APdamage')")
    ...
    if (script != getProperty(effect, "flags.wfrp2e.script"))
      updateData["flags.wfrp2e.script"] = script
