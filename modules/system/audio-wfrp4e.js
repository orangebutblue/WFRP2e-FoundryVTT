export default class WFRP_Audio {
  static PlayContextAudio(context) {
    this.MatchContextAudio(context).then(sound => {
      if (!sound || !sound.file) {
        console.warn("wfrp2e | Sound file not found for context: %o", context)
        return
      }
  ...
  static async MatchContextAudio(context) {
    if (!game.settings.get("wfrp2e", "soundPath") || !context)
      return {}

    try {
      let files, file, group;
      await FilePicker.browse("user", game.settings.get("wfrp2e", "soundPath")).then(resp => {
        files = resp.files
      })
