...
export default class WFRP_Tables {
  ...
  static async rollTable(tableKey, options = {}, column = null) {
    ...
    if (table) {
      ...
      let flags = rollResult.data.flags.wfrp2e || {}
      let result = {
      ...
      if (Object.keys(game.wfrp2e.config.hitLocationTables).includes(tableKey))
  ...
  static formatHitloc(result, roll) {
    let flags = result.data.flags.wfrp2e || {}
    return {
  ...
  static async rollToChat(table, options = {}, column = null, rollMode) {
    let chatOptions = game.wfrp2e.utility.chatDataSetup("", rollMode, true)
  ...
  static findTable(key, column) {
    let tables = game.tables.filter(i => i.getFlag("wfrp2e", "key") == key)

    // If more than one table with that key, and column is specified, return that column
    if (tables.length > 1 && column)
      return tables.find(i => i.getFlag("wfrp2e", "column") == column)
    ...
    else if (tables.length == 1 || tables.map(t => t.getFlag("wfrp2e", "column")).filter(t => t).length < 1)
      return tables[0]
  ...
  static promptColumn(table) {
    ...
    for (let c of tableObject.columns)
      prompt += `<div><a class = "table-click" data-table="${table}" data-column = "${c.getFlag("wfrp2e", "column")}"><i class="fas fa-list"></i> ${c.name}</a></div>`
