...
export default class TestWFRP {
  constructor(data, actor) {
    ...
    this.data = {
      preData: {
        ...
        testDifficulty: (typeof data.testDifficulty == "string" ? game.wfrp2e.config.difficultyModifiers[data.testDifficulty] : data.testDifficulty) || 0,
  ...
  runPostEffects() {
    ...
    Hooks.call("wfrp2e:rollTest", this, this.context.cardOptions)
  }
  ...
  async computeResult() {
    ...
    // ********** Success **********
      ...
      if (!game.settings.get("wfrp2e", "mooRangedDamage")) {
    ...
    if (this.preData.hitLocation) {
      if (this.preData.hitloc)
        this.result.hitloc = await game.wfrp2e.tables.rollTable("hitloc", { lookup: this.preData.hitloc, hideDSN: true });
      else
        this.result.hitloc = await game.wfrp2e.tables.rollTable("hitloc", { hideDSN: true });
    ...
    if (game.settings.get("wfrp2e", "criticalsFumblesOnAllTests") && !this.data.hitLocation) {
      if ((roll > target && roll % 11 == 0) || roll == 100 || roll == 99) {
        this.result.color_red = true;
  ...
  async postTest() {
    //@HOUSE
    if (game.settings.get("wfrp2e", "mooCriticalMitigation") && this.result.critical) {
      game.wfrp2e.utility.logHomebrew("mooCriticalMitigation")
      try {
        ...
        game.wfrp2e.utility.log("Error appyling homebrew mooCriticalMitigation: " + e)
      }
    }
    //@/HOUSE
  ...
  static recreate(data) {
    let test = new game.wfrp2e.rolls[data.preData.rollClass]()
    test.data = data
 ...
 */
  async renderRollCard({ newMessage = false } = {}) {
    ...
    if (game.settings.get("wfrp2e", "manualChatCards") && !this.message)
    ...
    else // Update message
    {
      ...
      if (game.user.isGM || this.message.isAuthor)
      {
        ...
        game.socket.emit("system.wfrp2e", { type: "updateMsg", payload: { id: this.message.id, updateData : chatOptions } })
  ...
  updateMessageFlags(updateData = {}) {
    ...
    else if (this.message)
    {
      ...
      game.socket.emit("system.wfrp2e", { type: "updateMsg", payload: { id: this.message.id, updateData: update} })
    }
  }
  ...
  _handleMiscasts(miscastCounter) {
    ...
    else if (!game.settings.get("wfrp2e", "mooCatastrophicMiscasts") && miscastCounter >= 3)
      this.result.majormis = game.i18n.localize("ROLL.MajorMis")

    //@HOUSE
    else if (game.settings.get("wfrp2e", "mooCatastrophicMiscasts") && miscastCounter >= 3) {
      game.wfrp2e.utility.logHomebrew("mooCatastrophicMiscasts")
      if (this.hasIngredient) {
