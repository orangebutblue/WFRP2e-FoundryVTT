...
export default class ChannelTest extends TestWFRP {
  ...
  computeTargetNumber() {
    ...
    {
      let skill = this.actor.getItemTypes("skill").find(s => s.name == `${game.i18n.localize("NAME.Channelling")} (${game.wfrp2e.config.magicWind[this.item.lore.value]})`)
      if (!skill)
  ...
  runPostEffects() {
    ...
    Hooks.call("wfrp2e:rollChannelTest", this, this.context.cardOptions)
  }
  ...
  postTest() {
    ...
    if (Number(SL) < 0 && game.settings.get("wfrp2e", "channelingNegativeSLTests"))
      SL = 0;
