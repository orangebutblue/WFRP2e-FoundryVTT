import TestWFRP from "./test-wfrp2e.js"
...
export default class CastTest extends TestWFRP {
  ...
  runPostEffects() {
    ...
    Hooks.call("wfrp2e:rollCastTest", this, this.context.cardOptions)
  }
  ...
  async computeResult() {
    ...
    if (this.result.outcome == "failure") {
      ...
      if (this.result.roll % 11 == 0 || this.result.roll == 100) {
        ...
        miscastCounter++;
        //@HOUSE
        if (this.result.roll == 100 && game.settings.get("wfrp2e", "mooCatastrophicMiscasts")) {
    ...
    else // Successful test, casted - determine overcast
    {
      ...
      //@HOUSE
      if (game.settings.get("wfrp2e", "mooCriticalChannelling")) {
        game.wfrp2e.utility.logHomebrew("mooCriticalChannelling")
  ...
  postTest() {
    ...
    if (this.item.cn.SL > 0) {
      ...
      if (this.result.castOutcome == "success" || !game.settings.get("wfrp2e", "mooCastAfterChannelling"))
        this.item.update({ "data.cn.SL": 0 })

      else if (game.settings.get("wfrp2e", "mooCastAfterChannelling")) {
        game.wfrp2e.utility.logHomebrew("mooCastAfterChannelling")
