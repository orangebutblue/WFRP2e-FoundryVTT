...
import EffectWfrp2e from "./effect-wfrp2e.js";
...
export default class SocketHandlers  {
    ...
    static applyEffects(data){
        if (!game.user.isUniqueGM)
            return
        game.wfrp2e.utility.applyEffectToTarget(data.payload.effect, data.payload.targets.map(t => new TokenDocument(t, {parent: game.scenes.get(data.payload.scene)})))
    }
    static applyOneTimeEffect(data){
        ...
        let actor = new ActorWfrp2e(data.payload.actorData)
        let effect = new EffectWfrp2e(data.payload.effect)
        try {
