...
export default function () {
  ...
  // Since IDs are maintained in WFRP2E, we have to clean actor imports from their IDs
  function WFRP2EImportFromJson(json) {
   ...
   // keep old functions
   CONFIG.Scene.documentClass.prototype.importFromJSON = WFRP2EImportFromJson;
   CONFIG.JournalEntry.documentClass.prototype.importFromJSON = WFRP2EImportFromJson;
   CONFIG.Actor.documentClass.prototype.importFromJSON = WFRP2EImportFromJson;
   CONFIG.Item.documentClass.prototype.importFromJSON = WFRP2EImportFromJson;
   CONFIG.RollTable.documentClass.prototype.importFromJSON = WFRP2EImportFromJson;
  ...
  TextEditor.enrichHTML = function (content, { secrets = false, documents = true, links = true, rolls = true, rollData = null } = {}) {
    ...
    if (documents) {
      ...
      const documentTypes = CONST.DOCUMENT_LINK_TYPES.concat("Compendium").concat(game.wfrp2e.config.PSEUDO_ENTITIES);
  ...
  TextEditor._createContentLink = function (match, type, target, name) {
    ...
    else if (game.wfrp2e.config.PSEUDO_ENTITIES.includes(type)) {
  ...
  Token.prototype.drawEffects = async function () {
    ...
    if (tokenEffects.length || actorEffects.length) {
      ...
      for (let f of actorEffects) {
        ...
        promises.push(this._drawEffect(f.data.icon, i, bg, w, tint, getProperty(f, "data.flags.wfrp2e.value")));
        i++;
      }
  ...
  Token.prototype._drawEffect = async function (src, i, bg, w, tint, value) {
    ...
    if (value) {
      let text = this.hud.effects.addChild(new PreciseText(value, game.wfrp2e.config.effectTextStyle))
      text.x = icon.x;
      text.y = icon.y;
...
*/
  ...
  Token.prototype.incrementCondition = async function (effect, { active, overlay = false } = {}) {
    ...
    if (!existing || Number.isNumeric(getProperty(existing, "data.flags.wfrp2e.value")))
