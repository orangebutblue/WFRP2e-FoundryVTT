...
import WFRP_Tables from "./tables-wfrp2e.js";
import ItemWfrp2e from "../item/item-wfrp2e.js";
...
export default class WFRP_Utility {
  ...
  static propertyStringToArray(propertyString, propertyObject)
  {
      ...
      for (let property of oldProperties) {
        ...
        newProperty.name = game.wfrp2e.utility.findKey(splitProperty, propertyObject)
        if (newProperty)
  ...
  static propertyStringToObject(propertyString, propertyObject)
  {
      ...
      return ItemWfrp2e._propertyArrayToObject(array, propertyObject)
  }
  ...
  static async  speciesCharacteristics(species, average, subspecies) {
    let characteristics = {};
    let characteristicFormulae = game.wfrp2e.config.speciesCharacteristics[species];
    if (subspecies && game.wfrp2e.config.subspecies[species][subspecies].characteristics)
      characteristicFormulae = game.wfrp2e.config.subspecies[species][subspecies].characteristics
    ...
    for (let char in game.wfrp2e.config.characteristics) {
      if (average) {
  ...
  static speciesSkillsTalents(species, subspecies) {
    ...
    skills = game.wfrp2e.config.speciesSkills[species]
    talents = game.wfrp2e.config.speciesTalents[species]

    if (subspecies && game.wfrp2e.config.subspecies[species][subspecies].skills)
      skills = game.wfrp2e.config.subspecies[species][subspecies].skills

    if (subspecies && game.wfrp2e.config.subspecies[species][subspecies].talents)
      talents = game.wfrp2e.config.subspecies[species][subspecies].talents
  ...
  static speciesMovement(species, subspecies) {
    let move = game.wfrp2e.config.speciesMovement[species];
    if (subspecies && game.wfrp2e.config.subspecies[species].movement)
      move = game.wfrp2e.config.subspecies[species].movement
    return move;
  }
  ...
  static getSystemEffects() {
    let systemEffects = duplicate(game.wfrp2e.config.systemEffects)

    Object.keys(systemEffects).map((key, index) => {
      systemEffects[key].obj = "systemEffects"
    })

    let symptomEffects = duplicate(game.wfrp2e.config.symptomEffects)
  ...
  {
    if (type == "skill")
      return game.wfrp2e.utility.findSkill(name)
    if (type == "talent")
      return game.wfrp2e.utility.findTalent(name)
    else
      return game.wfrp2e.utility.findItem(name, type)
  }
  ...
  static async findSkill(skillName) {
    ...
    let skillList = [];
    let packs = game.wfrp2e.tags.getPacksWithTag("skill")
  ...
  static async findTalent(talentName) {
    ...
    let talentList = [];
    let packs = game.wfrp2e.tags.getPacksWithTag("talent")
    for (let pack of packs) {
  ...
  static async findItem(itemName, itemType, location = null) {
    ...
    for (let p of game.wfrp2e.tags.getPacksWithTag(itemType)) {
  ...
  static async findAll(type) {
    ...
    for (let p of game.wfrp2e.tags.getPacksWithTag(type)) {
   ...
   */
  static qualityList() {
    let weapon = duplicate(game.wfrp2e.config.weaponQualities);
    let armor = duplicate(game.wfrp2e.config.armorQualities);
    let item = duplicate(game.wfrp2e.config.itemQualities);
   ...
   */
  static flawList() {
    let weapon = duplicate(game.wfrp2e.config.weaponFlaws);
    let armor = duplicate(game.wfrp2e.config.armorFlaws);
    let item = duplicate(game.wfrp2e.config.itemFlaws);
    let list = mergeObject(weapon, mergeObject(item, armor))
    return list;
  }
  ...
  static _calculateAdvCost(currentAdvances, type, modifier = 0) {
    ...
    if (index >= game.wfrp2e.config.xpCost[type].length)
      return game.wfrp2e.config.xpCost[game.wfrp2e.config.xpCost.length - 1] + modifier;
    return game.wfrp2e.config.xpCost[type][index] + modifier;
  }
  ...
  static postSymptom(symptom) {
    let symkey = WFRP_Utility.findKey(symptom.split("(")[0].trim(), game.wfrp2e.config.symptoms)
    let content = `<b>${symptom}</b>: ${game.wfrp2e.config.symptomDescriptions[symkey]}`;
    let chatOptions = {
      user: game.user.id,
    ...
    if (game.user.isGM) {
      content = `<b>${symptom} ${game.i18n.localize("Treatment")}</b>: ${game.wfrp2e.config.symptomTreatment[symkey]}`;
      chatOptions = {
        user: game.user.id,
        rollMode: game.settings.get("core", "rollMode"),
        content: content
  ...
  static postProperty(property) {
    let properties = mergeObject(WFRP_Utility.qualityList(), WFRP_Utility.flawList()),
      propertyDescr = Object.assign(duplicate(game.wfrp2e.config.qualityDescriptions), game.wfrp2e.config.flawDescriptions),
      propertyKey;
  ...
  static async allBasicSkills() {
    ...
    const packs = game.wfrp2e.tags.getPacksWithTag(["money", "skill"])
  ...
  static async allMoneyItems() {
    let moneyItems = []
    const packs = game.wfrp2e.tags.getPacksWithTag("money")
    ...
    for (let pack of packs) {
      ...
      let money = items.filter(t => Object.values(game.wfrp2e.config.moneyNames).map(n => n.toLowerCase()).includes(t.name.toLowerCase()))
  ...
  static alterDifficulty(difficulty, steps) {
    let difficulties = Object.keys(game.wfrp2e.config.difficultyLabels)
  ...
  static _replaceCustomLink(match, entityType, id, name) {
    ...
    switch (entityType) {
      ...
      case "Table":
        return `<a class = "table-click" data-table="${ids[0]}"><i class="fas fa-list"></i> ${(game.wfrp2e.tables.findTable(id)?.name && !name) ? game.wfrp2e.tables.findTable(id)?.name : name}</a>`
      case "Symptom":
      ...
      case "Corruption":
        return `<a class = "corruption-link" data-strength="${ids[0]}"><img src="systems/wfrp2e/ui/chaos.svg" height=15px width=15px style="border:none"> ${name ? name : id}</a>`
      case "Fear":
        return `<a class = "fear-link" data-value="${ids[0]}" data-name="${ids[1] || ""}"><img src="systems/wfrp2e/ui/fear.svg" height=15px width=15px style="border:none"> ${entityType} ${ids[0]}</a>`
      case "Terror":
        return `<a class = "terror-link" data-value="${ids[0]}" data-name="${ids[1] || ""}"><img src="systems/wfrp2e/ui/terror.svg" height=15px width=15px style="border:none"> ${entityType} ${ids[0]}</a>`
      case "Exp":
  ...
  /**
   * Collects data from the table click event and sends it to game.wfrp2e.tables to be rolled.
   *
  ...
  static async handleTableClick(event) {
    ...
    if (event.button == 0) {
      ...
      if (clickText.trim() == game.i18n.localize("ROLL.CritCast")) {
        html = game.wfrp2e.tables.criticalCastMenu($(event.currentTarget).attr("data-table"));
      }

      else if (clickText.trim() == game.i18n.localize("ROLL.TotalPower"))
        html = game.wfrp2e.tables.restrictedCriticalCastMenu();
      ...
      // Not really a table but whatever
        ...
        html = await game.wfrp2e.tables.formatChatRoll($(event.currentTarget).attr("data-table"),
          {
            modifier: modifier
    ...
    //   {
    //     new game.wfrp2e.apps.Wfrp2eTableSheet($(event.currentTarget).attr("data-table")).render(true)
    //   }
    // }
  }
  ...
  static handleConditionClick(event) {
    ...
    let condkey = WFRP_Utility.findKey(cond, game.wfrp2e.config.conditions, { caseInsensitive: true });
    let condName = game.wfrp2e.config.conditions[condkey];
    let condDescr = game.wfrp2e.config.conditionDescriptions[condkey];
 ...
 */
  ...
  static postCorruptionTest(strength) {
    renderTemplate("systems/wfrp2e/templates/chat/corruption.html", { strength }).then(html => {
  ...
  static postFear(value = 0, name = undefined) {
    ...
    if (name)
      title += ` - ${name}`
    renderTemplate("systems/wfrp2e/templates/chat/fear.html", { value, name, title }).then(html => {
  ...
  static postTerror(value = 1, name = undefined) {
    ...
    if (name)
      title += ` - ${name}`
    renderTemplate("systems/wfrp2e/templates/chat/terror.html", { value, name, title }).then(html => {
      ChatMessage.create({ content: html, speaker: { alias: name } });
    })
  }
  ...
  static postExp(amount, reason = undefined) {
    ...
    renderTemplate("systems/wfrp2e/templates/chat/experience.html", { title, amount, reason }).then(html => {
      ChatMessage.create({ content: html });
    })
  }
  ...
  static applyEffectToTarget(effect, targets) {
    ...
    if (game.user.isGM) {
      ...
      game.socket.emit("system.wfrp2e", { type: "applyEffects", payload: { effect, targets: [...targets].map(t => t.document.toObject()), scene: canvas.scene.id } })
    }
  ...
  static applyOneTimeEffect(effect, actor) {
      ...
      if (actor.hasPlayerOwner) {
        for (let u of game.users.contents.filter(u => u.active && !u.isGM)) {
          if (actor.data.permission.default >= CONST.DOCUMENT_PERMISSION_LEVELS.OWNER || actor.data.permission[u.id] >= CONST.DOCUMENT_PERMISSION_LEVELS.OWNER) {
            ...
            game.socket.emit("system.wfrp2e", { type: "applyOneTimeEffect", payload: { userId: u.id, effect: effectObj, actorData: actor.toObject() } })
            return
          }
        }
      }
    ...
    let func = new asyncFunction("args", getProperty(effect, "flags.wfrp2e.script")).bind({ actor, effect })
    func({ actor })
  }
  ...
  static invokeEffect(actor, effectId, itemId) {
    ...
    let asyncFunction = Object.getPrototypeOf(async function () { }).constructor
    let func = new asyncFunction("args", getProperty(effect, "flags.wfrp2e.script")).bind({ actor, effect, item })
  ...
  static _packageTables() {
    let tables = {}
    let tableValues = Object.values(game.wfrp2e.tables);
    let tableKeys = Object.keys(game.wfrp2e.tables);
  ...
  static async convertWFRPTable(tableId) {
    let table = game.wfrp2e.tables[tableId]
    let rollTable
    if (table.columns || table.multi)
    {
      ...
      if (table.multi)
      {
        for (let column of table.multi)
        {
          ...
          rollTableColumn["flags.wfrp2e.key"] = tableId
          rollTableColumn["flags.wfrp2e.column"] = column
    ...
    else
    {
      ...
      rollTable["flags.wfrp2e.key"] = tableId
  ...
  */
  ...
  {
    let keys = Object.keys(game.wfrp2e.config.rangeBands)
    let rangeKey = this.findKey(range, game.wfrp2e.config.rangeBands)
    let weaponRange = weapon.getFlag("wfrp2e", "optimalRange")
  ...
  static log(message) {
    console.log(`%cWFRP2E` + `%c | ${message}`, "color: gold", "color: unset");
  }
