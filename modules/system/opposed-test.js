...
export default class OpposedTest {
  ...
  _createTest(testData) {
    if (!testData)
      return testData
    let test = game.wfrp2e.rolls.TestWFRP.recreate(testData)
    test.data = testData
    return test
  }
  ...
  createUnopposedDefender(actor) {
    this.defenderTest = new game.wfrp2e.rolls.CharacteristicTest({
      item: "ws",
      SL: 0,
      target: 0,
      roll: 0,
...
*/
  checkPostModifiers() {
    ...
    if (game.settings.get("wfrp2e", "weaponLength") && this.attackerTest.weapon && this.defenderTest.weapon && this.attackerTest.weapon.attackType == "melee" && this.defenderTest.weapon.attackType == "melee") {
    ...
    try {
      ...
      if (attackerSL > defenderSL || (attackerSL === defenderSL && (attackerTest.target > defenderTest.target || (attackerTest.outcome == "success" && defenderTest.context.unopposed)))) {
        ...
        if (attackerTest.hitloc) {
          ...
          let remappedHitLoc = await game.wfrp2e.tables.rollTable(defender.details.hitLocationTable.value, { lookup: attackerTest.hitloc.roll, hideDSN: true })
      ...
      else // Defender won
      {
        ...
        if (defenderTest.result.champion || riposte) {
          ...
          this.defenderTest = game.wfrp2e.rolls.TestWFRP.recreate(attackerTest.data);
          this.attackerTest = game.wfrp2e.rolls.TestWFRP.recreate(temp)
          ...
          let hitloc = await game.wfrp2e.tables.rollTable(defenderTest.actor.details.hitLocationTable.value, {hideDSN : true})
      ...
      Hooks.call("wfrp2e:opposedTestResult", this, attackerTest, defenderTest)
  ...
  calculateOpposedDamage() {
    ...
    sizeDiff = game.wfrp2e.config.actorSizeNums[this.attackerTest.size] - game.wfrp2e.config.actorSizeNums[this.defenderTest.size]
    ...
    //@HOUSE
    if (game.settings.get("wfrp2e", "mooSLDamage")) {
      game.wfrp2e.utility.logHomebrew("mooSLDamage")
    ...
    {
      game.wfrp2e.utility.logHomebrew("mooRangedDamage")
    ...
    if (game.settings.get("wfrp2e", "mooSizeDamage"))
      return damage
